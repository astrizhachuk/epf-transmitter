...
// * Token - Строка - см. Константа.ТокенГитлаб
...
Функция ПолучитьПараметрыСоединения( Знач АдресСервера ) Экспорт
	...
	Результат.Token = НастройкиСервисов.ПолучитьТокенГитлаб();
...
Функция ПолучитьДанныеЗаполнения( Знач Источник ) Экспорт
	...
	Результат.CheckoutSHA = ПолучитьИдентификаторСобытия( ДанныеВнешнегоЗапроса );
	
	ЗаполнитьПроект( Результат, ДанныеВнешнегоЗапроса );
	
	Для Каждого Коммит Из ПолучитьКоммиты( ДанныеВнешнегоЗапроса ) Цикл
		
		Результат.Commits.Добавить( ПолучитьОписаниеКоммита(Коммит) );
		
	КонецЦикла;
	
	Для Каждого ОписаниеФайла Из ПолучитьИнформациюОбИзмененныхФайлах( ДанныеВнешнегоЗапроса ) Цикл
		
		Результат.ModifiedFiles.Добавить( ПолучитьОписаниеИзмененногоФайла(ОписаниеФайла.Id, ОписаниеФайла.FilePath) );
...
Функция ОбработатьЗапросСобытия( Знач Запрос ) Экспорт
	...
	Если ( НЕ СервисыHTTP.КодыСостояния().isOk(Ответ.StatusCode) ) Тогда
	...
	Если ( СервисыHTTP.КодыСостояния().isOk(Ответ.StatusCode) ) Тогда
...
Функция ПолучитьОписаниеКоммита( Знач Коммит )
	...
	Результат.Вставить( "Id", Коммит.Get("id") );
	Результат.Вставить( "Date", Коммит.Get("timestamp") );
...
// 	* Id - Строка - идентификатор коммита (json: commits[].id);
// 	* FilePath - Строка - относительный путь к файлу на сервере GitLab (json: commits[].modified[]),
// 							например, "а/б/в.epf"
//
...
// * FileName - Строка - имя файла (UTF-8);
// * BinaryData - Неопределено, ДвоичныеДанные - данные (файл);
// * ErrorInfo - Неопределено, ИнформацияОбОшибке - информация об ошибке, если она была при загрузке файла; 
//
Функция ОписаниеФайла()
	
	Результат = Новый Структура();
	Результат.Insert( "RAWFilePath", "" );
	Результат.Insert( "FileName", "" );
	Результат.Insert( "BinaryData", Неопределено );
	Результат.Insert( "ErrorInfo", Неопределено );
...
Функция ЗагрузитьФайл( Знач ПараметрыСоединения, Знач ПутьКФайлу )
	...
	Попытка
		...
		Результат.FileName = ПолучитьИмяФайла( Ответ );
	...
	Исключение
		...
		Результат.ErrorInfo = ФайлыВнешнегоХранилища.ПолучитьОшибкуЗагрузкиФайла( ПредставлениеОшибки, АдресСервера );
