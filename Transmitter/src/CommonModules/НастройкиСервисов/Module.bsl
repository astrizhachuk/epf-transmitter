...
// ПолучитьГлобальныеНастройкиВнешнегоСервиса возвращает набор параметров подключения
// к сервису внешней информационной базы.
// 
// Возвращаемое значение:
//  Структура - параметры подключения:
// * Пользователь - Строка - имя пользователя (макс. 256 символов);
// * Пароль - Строка - пароль (макс. 256 символов);
// * Таймаут - Число - таймаут соединения, сек (0 - таймаут не установлен);
//
Функция ПолучитьГлобальныеНастройкиВнешнегоСервиса() Экспорт
	
	Результат = Новый Структура();
	
	Результат.Вставить( "Пользователь", ПолучитьПользователяВнешнегоСервиса() );
	Результат.Вставить( "Пароль", ПолучитьПарольВнешнегоСервиса() );
	Результат.Вставить( "Таймаут", ПолучитьТаймаутВнешнегоСервиса() );
...
// УстановитьНастройкиСервисаОбработкиЗапросаГитлаб выполняет установку настроек сервиса обработки запроса
// с сервера Gitlab переданными значениями.
//
// Параметры:
...
// УстановитьНастройкиСервисаОбработкиЗапросаГитлаб выполняет установку настроек параметров соединения
// к сервисам информационной базы получателя по умолчанию переданными значениями.
//
// Параметры:
// 	Настройки - см. НастройкиСервисовКлиентСервер.НастройкиПараметровСоединенияКПолучателямПоУмолчанию
//
Процедура УстановитьНастройкиПараметровСоединенияКПолучателямПоУмолчанию( Знач Настройки ) Экспорт
...
// Устарела. ПолучитьНастройкиСервисаОбработкиЗапросаГитлаб возвращает текущие настройки сервиса
// обработки запроса с сервера Gitlab.
// 
// Возвращаемое значение:
...
// Устарела.
// ПолучитьНастройкиПараметровСоединенияКПолучателямПоУмолчанию возвращает текущие настройки параметров соединения
// к сервисам информационной базы получателя по умолчанию.
// 
// Возвращаемое значение:
// ФиксированнаяСтруктура - набор параметров:
// * ПользовательВнешнегоСервиса - см. ПолучитьПользователяВнешнегоСервиса
// * ПарольВнешнегоСервиса - см. ПолучитьПарольВнешнегоСервиса
// * ТаймаутВнешнегоСервиса - см. ПолучитьТаймаутВнешнегоСервиса
//
Функция ПолучитьНастройкиПараметровСоединенияКПолучателямПоУмолчанию() Экспорт
...
//
Функция ПолучитьПользователяВнешнегоСервиса()
	
	Результат = Константы.ПользовательВнешнегоСервиса.Получить();
...
//
Функция ПолучитьПарольВнешнегоСервиса()
	
	Результат = Константы.ПарольВнешнегоСервиса.Получить();
...
// 	Число - таймаут соединения, сек (0 - таймаут не установлен);
//
Функция ПолучитьТаймаутВнешнегоСервиса()
	
	Результат = Константы.ТаймаутВнешнегоСервиса.Получить();
...
Функция ПолучитьСообщениеСтатуса( Знач Сервис )
	...
	Если ( ОбработкаЗапросаВключена(Сервис) ) Тогда
		...
		Сообщение = НСтр( "ru = 'обработка запросов включена';en = 'request handler enabled'" );
		
	Иначе
		
		Сообщение = НСтр( "ru = 'обработка запросов отключена';en = 'request handler disabled'" );
		
	КонецЕсли;
	
	Возврат СервисыHTTP.ПолучитьСхемуСообщения( Сообщение );
