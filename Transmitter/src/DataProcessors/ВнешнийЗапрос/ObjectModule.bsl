...
// ПолучитьТипИсточника возвращает тип источника запроса.
// 
// Возвращаемое значение:
//  ПеречислениеСсылка.ИсточникиЗапроса - тип источника запроса.
//
Функция ПолучитьТипИсточника() Экспорт
...
// ПолучитьДобавленныеФайлы возвращает список с информацией обо всех добавленных файлах в разрезе коммитов.
// При отсутствии соответствующего коммита выбрасывает исключение.
// 
// Возвращаемое значение:
// 	Массив из Структура - добавленные файлы:
// * Идентификатор - ОпределяемыйТип.ИдентификаторКоммита - идентификатор коммита (commit SHA); 
// * Дата - Дата - дата добавления файла.
...
//
Функция ПолучитьДобавленныеФайлы() Экспорт
	
	Результат = ДобавленныеФайлы.Выгрузить();
	...
	Для Каждого Файл Из Результат Цикл
		...
		Если Коммит = Неопределено Тогда
			...
			// TODO сделать сообщение более дружелюбным? может добавить искомый хэш коммита?
...
// ПолучитьИзмененныеФайлы возвращает список с информацией обо всех измененных файлах в разрезе коммитов.
// При отсутствии соответствующего коммита выбрасывает исключение.
// 
// Возвращаемое значение:
// 	Массив из Структура - измененные файлы:
// * Идентификатор - ОпределяемыйТип.ИдентификаторКоммита - идентификатор коммита (commit SHA); 
// * Дата - Дата - дата изменения файла.
// * ПутьКФайлу - Строка - относительный путь к файлу внешнего репозитория,
// 							например, "Каталог с отчетами и обработками/Внешняя Обработка 1.epf".
//
Функция ПолучитьИзмененныеФайлы() Экспорт
	
	Результат = ИзмененныеФайлы.Выгрузить();
...
// ЗаполнитьСхемыМаршрутов формирует схемы маршрутов в формате JSON из двоичных данных
// и привязывает их к соответствующему коммиту.
//
// Параметры:
//   ОписанияФайлов - Массив из см. ФайловыеМодели.ФайлИзХранилища - список описаний файлов со схемами маршрутов.
//   ИмяФайла - Строка - имя файла со схемами маршрутов.
//
Процедура ЗаполнитьСхемыМаршрутов( Знач ОписанияФайлов, Знач ИмяФайла ) Экспорт
	...
	СводныеДанные = ОбщегоНазначенияВызовСервера.МассивВТаблицуЗначений( ОписанияФайлов );
	...
	Для Каждого Коммит Из ПолучитьКоммиты() Цикл
		...
		ОписаниеФайла = ОбщегоНазначенияВызовСервера.СтрокаТаблицыЗначенийВСтруктуру( МетаданныеСхем[0] ); // см. ФайловыеМодели.ФайлИзХранилища
		// BSLLS:LineLength-on
		
		ДобавитьСхемуМаршрутов( Коммит.Идентификатор, ПолучитьСтрокуИзДвоичныхДанных(ОписаниеФайла.ДвоичныеДанные) );
...
//
// Параметры:
//	Источник - ПеречислениеСсылка.ИсточникиЗапроса - источник запроса;
//
...
Функция ВСтруктуру()
	...
	Результат.ДобавленныеФайлы = ТаблицаЗначенийВМассив(ДобавленныеФайлы.Выгрузить());
	Результат.ИзмененныеФайлы = ТаблицаЗначенийВМассив(ИзмененныеФайлы.Выгрузить());
...
Процедура ЗаполнитьДанными( Знач ДанныеЗаполнения )
	...
	Для Каждого ДобавленныйФайл Из ДанныеЗаполнения.ДобавленныеФайлы Цикл
		
		ЗаполнитьЗначенияСвойств( ДобавленныеФайлы.Добавить(), ДобавленныйФайл );
		
	КонецЦикла;
	
	Для Каждого ИзмененныйФайл Из ДанныеЗаполнения.ИзмененныеФайлы Цикл
		
		ЗаполнитьЗначенияСвойств( ИзмененныеФайлы.Добавить(), ИзмененныйФайл );
