...
Сценарий: Просмотр фоновых заданий по идентификатору запроса
	...
	Делаем запрос от GitLab с дополнительным идентификатором запроса
	
		Пусть Я отправляю "Push Hook" запрос с токеном "Token1" и телом из файла "/test/requests/push-2b9949a2.json" для сервиса "/api/ru/hs/gitlab/events/push"
	...
	Запоминаем текущее состояние списка фоновых заданий
		...
		Когда я нажимаю на кнопку с именем 'OpenBackgrounds'
...
Сценарий: Обновление состояний фоновых заданий
	...
	Дано Я создаю Expectation из файла "/test/expectations/endpoints.json"
...
Сценарий: Принудительное завершение фонового задания
	...
	Дано Я создаю Expectation из файла "/test/expectations/endpoints.json"
	#И я перезаполняю константу "EndpointTimeout" значением "0"
