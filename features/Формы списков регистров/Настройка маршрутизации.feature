...
Функционал: Настройка маршрутизации файлов из списка внешних запросов

Как Пользователь
Я хочу иметь возможность открывать редактор маршрутизации из списка внешних запросов
Чтобы быстро переходить к редактированию маршрутов
...
Сценарий: Проверка открытия редактора маршрутизации из списка внешних запросов
	...
	* Открываем список внешних запросов

		Когда В командном интерфейсе я выбираю 'Интеграция с GitLab' 'Внешние запросы'
		Тогда открылось окно 'Внешние запросы'
	...
	* Находим нужную запись и открываем редактор
		...
		Когда в таблице 'Список' я перехожу к строке:
			...
			| "21.07.2020 9:23:30" | "1b9949a21e6c897b3dcb4dd510ddb5f893adae2f" | "Тест обработки запроса"      | "Merge branch 'feature/update-epf' into 'master'" |
		...
		Тогда таблица "Коммиты" стала равной:
			...
			| '21.07.2020 9:22:30'  | 'ef886bb4'                | 'deploy'                                              |
			| '16.03.2020 13:00:15' | '968eca17'                | 'first'                                               |
