...
Сценарий: Отображение истории событий при ошибке аутентификации
	...
	* Проверяем появившиеся изменения в истории событий
		...
		И таблица "ИсторияСобытий" содержит строки:
			...
			| 'Веб-сервис' | 'сайт'             | 'Информация' | 'Cоединение c HTTP-сервисом' | '200'                | 'Получен внешний запрос' |
...
Сценарий: Отображение истории событий при ошибках скачивания файлов
	...
	* Запоминаем текущее состояние формы
		...
		И Я в истории событий устанавливаю отбор "Сегодня"
		И я запоминаю количество строк таблицы "ИсторияСобытий" как "ДоВыполненияЗапроса"
		И Я запоминаю значение выражения '7' в переменную "Ожидание"
		Если переменная "ДоВыполненияЗапроса" имеет значение 0 Тогда
			И Я запоминаю значение выражения 'Дата(НачалоДня(ТекущаяДата()))' в переменную "ДатаПоследнейЗаписи"
		Иначе
			И я запоминаю значение поля с именем 'ИсторияСобытийДата' таблицы 'ИсторияСобытий' как "ДатаПоследнейЗаписи"
	...
	* Проверяем появившиеся изменения в истории событий
		...
		И таблица "ИсторияСобытий" содержит строки по шаблону:
			| 'Событие'          | 'Имя пользователя' | 'Уровень'    | 'Приложение'                 | 'Код состояния HTTP' | 'Комментарий'                                                                                    |
			| 'Обработка данных' | 'сайт'             | 'Информация' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние запросы: Данные сохранены'                                                |
			| 'Обработка данных' | 'сайт'             | 'Ошибка'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Ошибка загрузки файла: URL: http://mockserver:1080*: Описание ошибки: NOT_FOUND*' |
			| 'Обработка данных' | 'сайт'             | 'Ошибка'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Ошибка загрузки файла: URL: http://mockserver:1080*: Описание ошибки: NOT_FOUND*' |
			| 'Веб-сервис'       | 'сайт'             | 'Информация' | 'Cоединение c HTTP-сервисом' | '200'                | 'Обработка запроса завершена'                                                                    |
			| 'Веб-сервис'       | 'сайт'             | 'Информация' | 'Cоединение c HTTP-сервисом' | '200'                | 'Получен внешний запрос'                                                                         |
			| 'Обработка данных' | 'сайт'             | 'Информация' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Запущено заданий на отправку файлов: 0'                                           |
			| 'Обработка данных' | 'сайт'             | 'Информация' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние файлы: Данные сохранены'                                                  |
...
Сценарий: Отображение истории событий при отсутствии маршрутизации
	...
	* Запоминаем текущее состояние формы
		...
		И Я в истории событий устанавливаю отбор "Сегодня"
		И я запоминаю количество строк таблицы "ИсторияСобытий" как "ДоВыполненияЗапроса"
		И Я запоминаю значение выражения '12' в переменную "Ожидание"
		Если переменная "ДоВыполненияЗапроса" имеет значение 0 Тогда
			И Я запоминаю значение выражения 'Дата(НачалоДня(ТекущаяДата()))' в переменную "ДатаПоследнейЗаписи"
		Иначе
			И я запоминаю значение поля с именем 'ИсторияСобытийДата' таблицы 'ИсторияСобытий' как "ДатаПоследнейЗаписи"
	...
	* Проверяем появившиеся изменения в истории событий
		...
		И таблица "ИсторияСобытий" содержит строки по шаблону:
			| 'Событие'          | 'Имя пользователя' | 'Уровень'        | 'Приложение'                 | 'Код состояния HTTP' | 'Комментарий'                                                                                    |
			| 'Веб-сервис'       | 'сайт'             | 'Информация'     | 'Cоединение c HTTP-сервисом' | '200'                | 'Получен внешний запрос'                                                                         |
			| 'Маршрутизация'    | 'сайт'             | 'Предупреждение' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: 1b9949a2: Внешняя Обработка 1.epf: Не задан маршрут доставки файла'               |
			| 'Маршрутизация'    | 'сайт'             | 'Предупреждение' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: 1b9949a2: Внешняя Обработка 1.epf: Не найдена схема маршрутизации'                |
			| 'Маршрутизация'    | 'сайт'             | 'Предупреждение' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: 1b9949a2: Внешняя Обработка 3.epf: Не задан маршрут доставки файла'               |
			| 'Маршрутизация'    | 'сайт'             | 'Предупреждение' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: 1b9949a2: Внешняя Обработка 3.epf: Не найдена схема маршрутизации'                |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Запущено заданий на отправку файлов: 0'                                           |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние файлы: Данные сохранены'                                                  |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние запросы: Данные сохранены'                                                |
			| 'Обработка данных' | 'сайт'             | 'Ошибка'         | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Ошибка загрузки файла: URL: http://mockserver:1080*: Описание ошибки: NOT_FOUND*' |
...
Сценарий: Отображение истории событий при успешной обработке внешнего запроса
	...
	* Проверяем что записи в истории событий появились
		...
		И таблица "ИсторияСобытий" содержит строки:
			| 'Событие'          | 'Имя пользователя' | 'Уровень'        | 'Приложение'                 | 'Код состояния HTTP' | 'Комментарий'                                                                      |
			| 'Веб-сервис'       | 'сайт'             | 'Информация'     | 'Cоединение c HTTP-сервисом' | '200'                | 'Получен внешний запрос'                                                           |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Запущено заданий на отправку файлов: 2'                             |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние запросы: Данные сохранены'                                  |
			| 'Обработка данных' | 'сайт'             | 'Информация'     | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: Внешние файлы: Данные сохранены'                                    |
			| 'Маршрутизация'    | 'сайт'             | 'Предупреждение' | 'Фоновое задание'            | ''                   | '[ 1b9949a2 ]: 1b9949a2: Внешняя Обработка 3.epf: Не задан маршрут доставки файла' |
			| 'Веб-сервис'       | 'сайт'             | 'Информация'     | 'Cоединение c HTTP-сервисом' | '200'                | 'Обработка запроса завершена'                                                      |
