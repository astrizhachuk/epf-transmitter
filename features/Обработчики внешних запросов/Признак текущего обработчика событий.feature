...
Сценарий: Выделение событий, относящихся к текущему обработчику
	...
	* Проверяем что записи в истории событий появились
		...
		И таблица "ИсторияСобытий" содержит строки:
			...
			| 'Веб-сервис'       | 'Получен запрос от сервиса GitLab'                                                    | 'Нет'                |
			| 'Обработка данных' | '[ 1b9949a21e6c897b3dcb4dd510ddb5f893adae2f ]: Запущено заданий на отправку файла: 2' | 'Да'                 |
			| 'Обработка данных' | '[ 1b9949a21e6c897b3dcb4dd510ddb5f893adae2f ]: Внешние запросы: Данные сохранены'     | 'Да'                 |
			| 'Обработка данных' | '[ 1b9949a21e6c897b3dcb4dd510ddb5f893adae2f ]: Внешние файлы: Данные сохранены'       | 'Да'                 |
			| 'Веб-сервис'       | 'Обработка запроса от сервиса GitLab завершена'                                       | 'Нет'                |
