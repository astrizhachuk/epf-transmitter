...
Сценарий: Редактирование элемента справочника "Обработчики событий"
	...
	И в поле 'Секретный ключ (Secret Token)' я ввожу текст 'Секретный ключ2'
...
Сценарий: Полное удаление элемента справочника "Обработчики событий"
	...
	И я нажимаю на кнопку 'Да'
	Тогда открылось окно '1С:Предприятие'
	И элемент формы с именем "ErrorInfo" стал равен 'Нарушение прав доступа!  Попробуйте уточнить параметры действия или обратиться к администратору приложения для предоставления прав.'
	И я нажимаю на кнопку 'OK'
	Тогда таблица "Список" стала равной:
		| 'Наименование'  | 'Код'       | 'Секретный ключ (Secret Token)' |
		| 'Наименование1' | '000000001' | 'Секретный ключ1'               |

Сценарий: Дублирование элементов справочника "Обработчики событий" по секретному ключу
	Дано Я добавляю новый обработчик событий "Наименование1" с ключом "Секретный ключ1"
	И Я добавляю новый обработчик событий "Наименование2" с ключом "Секретный ключ1"
	Тогда таблица "Список" стала равной:
		| 'Наименование'  | 'Код'       | 'Секретный ключ (Secret Token)' |
		| 'Наименование1' | '000000001' | 'Секретный ключ1'               |
