...
Сценарий: Загрузка истории событий с выбором по периоду "Сегодня"
	...
	Проверяем что записи истории событий появились
		...
		# Ошибка в VA. Не работает \n в шаге
		# | 'Информация' | 'Фоновое задание'            | '200'                | 'ОтправкаДанныхПолучателю'           | 'сайт'             | '[ 1b9949a21e6c897b3dcb4dd510ddb5f893adae2f ]: URL сервиса доставки: http://mock-server:1080/receiver3; файл: Внешняя Обработка 1.epf; текст ответа:\n{"type":"info","message":"Успех"}' |
		# | 'Информация' | 'Фоновое задание'            | '200'                | 'ОтправкаДанныхПолучателю'           | 'сайт'             | '[ 1b9949a21e6c897b3dcb4dd510ddb5f893adae2f ]: URL сервиса доставки: http://mock-server:1080/receiver1; файл: Внешняя Обработка 1.epf; текст ответа:\n{"type":"info","message":"Успех"}' |
		И таблица "EventsHistory" содержит строки:
			| 'Уровень'    | 'Приложение'                 | 'Код состояния HTTP' | 'Представление события'              | 'Имя пользователя' |
			| 'Информация' | 'Фоновое задание'            | '200'                | 'ОтправкаДанныхПолучателю'           | 'сайт'             |
			| 'Информация' | 'Фоновое задание'            | '200'                | 'ОтправкаДанныхПолучателю'           | 'сайт'             |
