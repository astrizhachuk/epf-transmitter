...
Сценарий: Я проверяю открытие Merge Request в браузере
	...
	Тогда я выполняю код встроенного языка
		|'Контекст.ЗапущенныеПроцессы = Ванесса.ВнешняяКомпонентаДляСкриншотов.ПолучитьСписокПроцессов(Ложь); '|
		|'Ванесса.ПроверитьВхождение(Контекст.ЗапущенныеПроцессы, "http://www.example.org");                  '|
