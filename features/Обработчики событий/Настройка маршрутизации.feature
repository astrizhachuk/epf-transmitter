...
Сценарий: Я редактирую сохраненные настройки маршрутизации
	...
	Редактируем текст по одной строке (некорректный формат)
		...
		И элемент с именем "CommitsJSONRouting" доступен только для просмотра
		И я устанавливаю флаг 'Пользовательский вариант'
		И в поле 'CommitsJSONRouting' я ввожу текст 'простой текст не в формате JSON'
	...
	Редактируем текст по одной строке (JSON-формат)
		...
		И элемент с именем "CommitsJSONRouting" доступен только для просмотра
		И я устанавливаю флаг 'Пользовательский вариант'
		И в поле 'CommitsJSONRouting' я ввожу текст '{"имя": "пользовательский вариант"}'
		И я нажимаю на кнопку 'Сохранить JSON'
	...
	Проверяем что значение на форме не сбрасывается при переходе на другую строку
		...
		И элемент формы с именем "CommitsJSONRouting" стал равен '{"имя": "пользовательский вариант"}'
		И элемент формы с именем "CommitsIsUserSettings" стал равен 'Да'
		И элемент с именем "CommitsJSONRouting" доступен не только для просмотра
		И я жду доступности элемента с именем "СохранитьJSON" в течение 2 секунд
		И я закрываю окно 'Настройка маршрутизации'
		И я жду закрытия окна 'Настройка маршрутизации' в течение 2 секунд
...
Сценарий: Я перезаписываю пользовательский вариант настройки маршрутизации
	...
	Редактируем ту же настройку еще раз с подтверждением перезаписи данных
		...
		И в таблице "Commits" я перехожу к строке:
			| 'commit_sha'                               |
			| "ef886bb4e372250d8212387350f7e139cbe5a1af" |
		И значение поля "CommitsJSONRouting" содержит текст '{"имя": "пользовательский вариант"}'

		И в поле 'CommitsJSONRouting' я ввожу текст '{"имя": "пользовательский вариант2"}'
		И я нажимаю на кнопку 'Сохранить JSON'
		Тогда появилось предупреждение, содержащее текст "Пользовательские настройки уже существуют, перезаписать?"
		...
		И в таблице "Commits" я перехожу к строке:
			| 'commit_sha'                               |
			| 'ef886bb4e372250d8212387350f7e139cbe5a1af' |
		И значение поля "CommitsJSONRouting" содержит текст '{"имя": "пользовательский вариант2"}'
		И элемент с именем "CommitsJSONRouting" доступен не только для просмотра
	...
	Редактируем ту же настройку еще раз с отказом от перезаписи данных
		...
		И в поле 'CommitsJSONRouting' я ввожу текст '{"имя": "пользовательский вариант3"}'
		И я нажимаю на кнопку 'Сохранить JSON'
		Тогда появилось предупреждение, содержащее текст "Пользовательские настройки уже существуют, перезаписать?"
...
Сценарий: Я удаляю пользовательский вариант настройки маршрутизации
	...
	Отказываемся от сброса настроек при снятии флага "Пользовательский вариант"
		...
		Тогда появилось предупреждение, содержащее текст "Сбросить настройки маршрутизации на значения по умолчанию?"
		И я нажимаю на кнопку 'Нет'
