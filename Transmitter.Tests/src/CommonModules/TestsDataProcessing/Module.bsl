...
Procedure BeginDataProcessing(Фреймворк) Export
	...
	ОбработчикСобытия = TestsWebhooksServer.AddExternalRequestHandler("ЮнитТест1", "РучнойЗапуск");
...
Procedure BeginDataProcessingHandleRequest(Фреймворк) Export
	...
	// given
	ExternalRequestHandlersCleanUp();
	ОбработчикСобытия = TestsWebhooksServer.AddExternalRequestHandler("ЮнитТест1", "Webhook");
...
//	JOB_WAS_STARTED_MESSAGE = НСтр("ru = 'фоновое задание уже запущено.';en = 'background job is already running.'");	
	...
	EventLogFilter = EventLogFilterByData(ExternalRequestHandler.Ref, "Error");
	
//	ОтборЖурналаРегистрации = ОтборЖурналаРегистрации(EVENT_MESSAGE, "Предупреждение");
//	ОбработчикСобытия = TestsWebhooksServer.AddExternalRequestHandler("ЮнитТест1", "РучнойЗапуск");
	// when
	Результат = DataProcessing.RunBackgroundJob(ExternalRequestHandler.Ref, CheckoutSHA);
	Результат = DataProcessing.RunBackgroundJob(ExternalRequestHandler.Ref, CheckoutSHA);
	Pause(3);
	...
	// then
	Фреймворк.ПроверитьРавенство(Результат, Неопределено);
	Фреймворк.AssertStringContains(EventLog[0].Comment, CheckoutSHA);	
