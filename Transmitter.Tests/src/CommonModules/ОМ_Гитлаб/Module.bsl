...
Процедура ПолучитьДанныеЗаполнения() Экспорт
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		...
		.Свойство("ProjectURL").Равно("http://example.com/path/")
		.Свойство("ServerURL").Равно("http://example.com")
		.Свойство("CheckoutSHA").Равно("123456789")
		.Свойство("Commits")
			.ИмеетТип("Массив")
			.ИмеетДлину(2)
		.Свойство("Commits[0]").ИмеетТип("Структура")
		.Свойство("Commits[0].Id").Равно("1b11")
		.Свойство("Commits[0].Date")
			.ИмеетТип("Дата")
			.Равно(Дата(2020, 07, 21, 09, 23, 30))
		.Свойство("Commits[1].Id").НеРавно("1b11")
		.Свойство("ModifiedFiles")
			.ИмеетТип("Массив")
			.ИмеетДлину(2)
		.Свойство("ModifiedFiles[0]").ИмеетТип("Структура")
		.Свойство("ModifiedFiles[0].Id").Равно("1b11")
		.Свойство("ModifiedFiles[0].FilePath").Равно("1.epf")
		.Свойство("ModifiedFiles[1].Id").Равно("1b11")
...
Процедура ПолучитьДанныеЗаполненияПриОтсутствииДанныхВФорматеJSONВозвращаетМинимальныйНаборДанных(Знач Параметр) Экспорт
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		...
		.Свойство("ProjectURL").НеЗаполнено()
		.Свойство("ServerURL").НеЗаполнено()
		.Свойство("CheckoutSHA").НеЗаполнено()
		.Свойство("Commits").НеЗаполнено()
...
Процедура ОбработатьЗапросСобытияВозвращает200() Экспорт
	...
	ВнешнийЗапрос.ProjectURL = ОМ_Тесты.ЮТест().Данные().СлучайнаяСтрока();
	...
	Конструктор
		...
		.Установить("ProjectURL", ВнешнийЗапрос.ProjectURL)
		.Установить("Token", Токен)
		.Записать( , Истина);
...
Процедура ОбработатьЗапросСобытияВозвращаетОшибку404ПриОстутствииОбработчикаСАдресомПроекта() Экспорт
	...
	Конструктор
		.ФикцияОбязательныхПолей()
		.Установить("ProjectURL", ОМ_Тесты.ЮТест().Данные().СлучайнаяСтрока())
		.Записать( , Истина);
