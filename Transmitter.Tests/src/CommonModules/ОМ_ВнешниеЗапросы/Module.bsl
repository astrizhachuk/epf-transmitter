...
Процедура ОпределитьФайлыКЗагрузкеПриИзмененииФайлаВРазныхКоммитахВозвращаетПоследнийПоДатеФайл() Экспорт
	...
	Тест.ВнешнийЗапрос.ModifiedFiles = ОМ_Тесты.ЮТКоллекции().ЗначениеВМассиве(
		НовыйИзмененныйФайлПереопределяемый(1, Тест.ВнешнийЗапрос.Коммиты[0].Идентификатор, Файл.ПутьКФайлу),
		НовыйИзмененныйФайлПереопределяемый(2, Тест.ВнешнийЗапрос.Коммиты[1].Идентификатор, Файл.ПутьКФайлу)
		);
