...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.ДобавитьТест("ОтправитьФайлыОбрабатываетОшибкуСозданияФоновогоЗаданияОтправкиФайла")
...
Процедура ОбработатьЗапрос() Экспорт
	...
	ВнешнийЗапрос.CheckoutSHA = ОМ_ТестовыеДанные.ПолучитьИдентификаторКоммита();
...
Процедура ОтправитьФайлыИсключаетИзОтправкиНераспределенныеФайлы() Экспорт
	...
	ФайлыКОтправке[0].Routes = ОМ_Тесты.ЮТКоллекции().ЗначениеВМассиве(ОМ_Тесты.ЮТест().Данные().СлучайнаяСтрока());
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат[0].Параметры[0])
		.ИмеетДлину(1)
		.Свойство("[0].Routes").Заполнено();

КонецПроцедуры

Процедура ОтправитьФайлыОбрабатываетОшибкуСозданияФоновогоЗаданияОтправкиФайла() Экспорт
	...
	Идентификатор = ОМ_ТестовыеДанные.ПолучитьИдентификаторКоммита();
	Файлы = ОМ_Тесты.ЮТКоллекции().ЗначениеВМассиве(ОписаниеФайла());
	РезультатОтправки = ОМ_Тесты.ЮТКоллекции().ЗначениеВМассиве(ОписаниеРезультатаОтправкиФайла());
	РезультатОтправки[0].ErrorInfo = ОМ_ТестовыеДанные.ПолучитьИнформациюОбОшибке("текст ошибки");
	
	ОМ_Тесты.Мокито().Обучение(Получатели)
		.Когда("ОтправитьФайлыВФоне").Вернуть(РезультатОтправки);
	ОМ_Тесты.Мокито().Прогон();
	
	// when
	Результат = ОбработкаДанных.ОтправитьФайлы(Обработчик, Идентификатор, Тест.СериализованныйВнешнийЗапрос, Файлы);
...
Процедура ПолучитьФоновыеЗаданияПоИдентификатору() Экспорт
	...
	// given
	Идентификатор = ОМ_Тесты.ЮТест().Данные().СлучайнаяСтрока();

	ФоновыеЗадания.Выполнить( "ОбработкаДанных.ОтправитьФайлы", , Идентификатор );	
	ФоновыеЗадания.Выполнить( "ОбработкаДанных.ЗагрузитьИОтправитьФайлы", , Идентификатор );
	ФоновыеЗадания.Выполнить( "Получатели.ОтправитьФайл", , Идентификатор + "|..." );
	
	// when
	Результат = ОбработкаДанных.ПолучитьФоновыеЗаданияПоИдентификатору(Идентификатор);
	
	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		.ИмеетТип("Массив")
		.ИмеетДлину(3)
		.Свойство("[0].Ключ").Равно(Идентификатор)
		.Свойство("[1].Ключ").НачинаетсяС(Идентификатор)
		.Свойство("[2].Ключ").НачинаетсяС(Идентификатор);
...
Функция Заглушка(Знач СхемыМаршрутов = Неопределено)
	...
	ВнешнийЗапрос.Вставить("Routes", Новый Массив());
...
Функция ОписаниеРезультатаОтправкиФайла()
	
	Результат = Новый Структура();
	Результат.Вставить( "BackgroundJob", Неопределено );
	Результат.Вставить( "ErrorInfo", Неопределено );
