...
Процедура ЗаписатьДанные() Экспорт
	...
	// when
	РегистрыСведений.ВнешниеЗапросы.ЗаписатьДанные(ФэйковыйОбработчик, Идентификатор, Данные);
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(Записи)
		.ИмеетДлину(1)
		.Свойство("[0].RequestHandler").Равно(ФэйковыйОбработчик)
		.Свойство("[0].CheckoutSHA").Равно(Идентификатор)
...
Процедура НайтиДанные(Знач Данные) Экспорт
	...
	Запись.RequestHandler = ФэйковыйОбработчик;
