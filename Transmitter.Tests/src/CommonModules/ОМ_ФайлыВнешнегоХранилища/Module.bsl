...
Функция ПолучитьОписаниеФайла(Знач Тест, Знач ПутьКФайлу, Знач Позиция, Действие)
	...
	Результат.Вставить( "Date", Тест.ВнешнийЗапрос.Коммиты[Позиция].Дата );
	Результат.Вставить( "CommitSHA", Тест.ВнешнийЗапрос.Коммиты[Позиция].Идентификатор );
...
Функция ПараметрыХранилища(Знач Проект, Знач Сервис)
	...
	Результат.Вставить( "ПараметрыСоединения", Новый Структура("URL, Токен, Timeout", "", "", 0) );
			...
			ОМ_Тесты.ЮТест().Предикат()
				.Свойство("RAWFilePath")
					.Равно(ПолучитьПутьКИсходномуФайлу(
						Тест.ВнешнийЗапрос.ProjectId, Тест.ВнешнийЗапрос.Коммиты[ПозицияВКоммитах].Идентификатор, Файл.FilePathEncoded))
				.Свойство("FileName")
					.Равно(Файл.FileName)
				.Свойство("FilePath")
					.Равно(Файл.FilePath)
