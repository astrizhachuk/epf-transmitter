...
Процедура ИсполняемыеСценарии() Экспорт
	...
	ЮТТесты
		...
		.ДобавитьТест("УстановитьИсточникЗапросаВозвращаетПустоеЗначениеПриНеверномТипеПараметра")
			.СПараметрами(1)
			.СПараметрами("что-то")
		.ДобавитьТест("ПолучитьТипИсточника")
		...
		.ДобавитьТест("ПолучитьДобавленныеФайлы")
		.ДобавитьТест("ПолучитьДобавленныеФайлыПриИхОтстутствииВозвращаетПустуюКоллекцию")
		.ДобавитьТест("ПолучитьДобавленныеФайлыВыбрасываетИсключениеПриОтсутствииИсходногоКоммита")
		.ДобавитьТест("ПолучитьИзмененныеФайлы")
		.ДобавитьТест("ПолучитьИзмененныеФайлыПриИхОтстутствииВозвращаетПустуюКоллекцию")
		.ДобавитьТест("ПолучитьИзмененныеФайлыВыбрасываетИсключениеПриОтсутствииИсходногоКоммита")
...
Процедура УстановитьИсточникЗапроса() Экспорт
	...
	// when
	ВнешнийЗапрос.УстановитьИсточникЗапроса(Перечисления.ИсточникиЗапроса.Гитлаб);

	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(ВнешнийЗапрос)
		.Свойство("ТипИсточника")
			.Равно(Перечисления.ИсточникиЗапроса.Гитлаб);
...
Процедура УстановитьИсточникЗапросаВозвращаетПустоеЗначениеПриНеверномТипеПараметра(Знач Параметр) Экспорт
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(ВнешнийЗапрос)
		.Свойство("ТипИсточника")
			.ИмеетТип("ПеречислениеСсылка.ИсточникиЗапроса")
			.НеЗаполнено();
	
КонецПроцедуры

Процедура ПолучитьТипИсточника() Экспорт
	...
	// when
	Результат = ВнешнийЗапрос.ПолучитьТипИсточника();

	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		.ИмеетТип("ПеречислениеСсылка.ИсточникиЗапроса")
		.Равно(Тест.ВнешнийЗапрос.ТипИсточника);
...
Процедура ПолучитьДобавленныеФайлы() Экспорт
	...
	// when
	Результат = ВнешнийЗапрос.ПолучитьДобавленныеФайлы();
	
	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		.ИмеетДлину(1)
		.Свойство("[0].Идентификатор").Равно(Тест.ВнешнийЗапрос.ДобавленныеФайлы[0].Идентификатор)
		.Свойство("[0].Дата").Равно(Тест.ВнешнийЗапрос.Коммиты[0].Дата)
		.Свойство("[0].ПутьКФайлу").Равно(Тест.ВнешнийЗапрос.ДобавленныеФайлы[0].ПутьКФайлу);
	
КонецПроцедуры

Процедура ПолучитьДобавленныеФайлыПриИхОтстутствииВозвращаетПустуюКоллекцию() Экспорт
	
	// given
	Тест = Заглушка();
	ВнешнийЗапрос = ОМ_ТестовыеДанные.ПолучитьФейковыйВнешнийЗапрос(Тест.ВнешнийЗапрос);
	ВнешнийЗапрос.ДобавленныеФайлы.Очистить();
...
Процедура ПолучитьДобавленныеФайлыВыбрасываетИсключениеПриОтсутствииИсходногоКоммита() Экспорт
	...
	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(ВнешнийЗапрос)
		.Метод("ПолучитьДобавленныеФайлы")
		.ВыбрасываетИсключение(Логи.Сообщения().NO_COMMIT);
	
КонецПроцедуры

Процедура ПолучитьИзмененныеФайлы() Экспорт
	...
	// when
	Результат = ВнешнийЗапрос.ПолучитьИзмененныеФайлы();
	
	// then
	ОМ_Тесты.ЮТест().ОжидаетЧто(Результат)
		.ИмеетДлину(3)
		.Свойство("[0].Идентификатор").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[0].Идентификатор)
		.Свойство("[0].Дата").Равно(Тест.ВнешнийЗапрос.Коммиты[0].Дата)
		.Свойство("[0].ПутьКФайлу").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[0].ПутьКФайлу)
		.Свойство("[1].Идентификатор").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[1].Идентификатор)
		.Свойство("[1].Дата").Равно(Тест.ВнешнийЗапрос.Коммиты[1].Дата)
		.Свойство("[1].ПутьКФайлу").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[1].ПутьКФайлу)
		.Свойство("[2].Идентификатор").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[2].Идентификатор)
		.Свойство("[2].Дата").Равно(Тест.ВнешнийЗапрос.Коммиты[2].Дата)
		.Свойство("[2].ПутьКФайлу").Равно(Тест.ВнешнийЗапрос.ИзмененныеФайлы[2].ПутьКФайлу);
	
КонецПроцедуры

Процедура ПолучитьИзмененныеФайлыПриИхОтстутствииВозвращаетПустуюКоллекцию() Экспорт
	
	// given
	Тест = Заглушка();
	ВнешнийЗапрос = ОМ_ТестовыеДанные.ПолучитьФейковыйВнешнийЗапрос(Тест.ВнешнийЗапрос);
	ВнешнийЗапрос.ИзмененныеФайлы.Очистить();
...
Процедура ПолучитьИзмененныеФайлыВыбрасываетИсключениеПриОтсутствииИсходногоКоммита() Экспорт
...
Процедура ЗаполнитьСхемыМаршрутов() Экспорт
	...
	Файлы = ОМ_Тесты.Мокито().МассивПараметров(
		ПолучитьОписаниеФайла(Тест, Файл, Тест.ВнешнийЗапрос.Коммиты[0].Идентификатор, Перечисления.ВидыИзмененийФайла.Добавлен),
		ПолучитьОписаниеФайла(Тест, ФайлСхемы, Тест.ВнешнийЗапрос.Коммиты[0].Идентификатор, Перечисления.ВидыИзмененийФайла.ПустаяСсылка()),
		ПолучитьОписаниеФайла(Тест, ФайлСхемы, "какой-то идентификатор коммита", Перечисления.ВидыИзмененийФайла.ПустаяСсылка()),
		ПолучитьОписаниеФайла(Тест, ФайлСхемы, Тест.ВнешнийЗапрос.Коммиты[1].Идентификатор, Перечисления.ВидыИзмененийФайла.ПустаяСсылка(), "тут была ошибка"));
	...
	ОМ_Тесты.ЮТест().ОжидаетЧто(ВнешнийЗапрос)
		.Свойство("СхемыМаршрутов")
			...
			.ИмеетДлину(1)
		.Свойство("СхемыМаршрутов[0].Идентификатор").Равно(Тест.ВнешнийЗапрос.Коммиты[0].Идентификатор)
		.Свойство("СхемыМаршрутов[0].Представление").Равно(ФайлСхемы.Данные)
		.Свойство("СхемыМаршрутов[0].Пользовательская").ЭтоЛожь();
