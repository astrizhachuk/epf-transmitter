...
Процедура ПередачаДвоичныхДанныхБезПараметровДоставкиОтсутствуютПараметрыСобытия(Фреймворк) Экспорт
	...
	Исключение
		...
		Фреймворк.ПроверитьВхождение(ИнформацияОбОшибке.Описание, "В структуре ожидалось свойство Адрес (тип Строка)");
	КонецПопытки;
	
	ЖурналРегистрации = СобытияЖурналаРегистрации(ОтборЖурналаРегистрации);
	Фреймворк.ПроверитьВхождение(ЖурналРегистрации[0].Комментарий, "В структуре ожидалось свойство Адрес (тип Строка)");	
	Фреймворк.ПроверитьИстину(СтрНачинаетсяС(ЖурналРегистрации[0].Комментарий, "Недопустимое значение параметра"));
...
Процедура ПередачаДвоичныхДанныхБезПараметровДоставкиЕстьПараметрыСобытия(Фреймворк) Экспорт
	...
	ЖурналРегистрации = СобытияЖурналаРегистрации(ОтборЖурналаРегистрации);
	Фреймворк.ПроверитьИстину(СтрНачинаетсяС(ЖурналРегистрации[0].Комментарий, "[ какой-то коммит... ]: Недопустимое значение параметра"));
	Фреймворк.ПроверитьВхождение(ЖурналРегистрации[0].Комментарий, "В структуре ожидалось свойство Адрес (тип Строка)");
...
Процедура ПередачаДвоичныхДанныхБезПараметровДоставкиВФоне(Фреймворк) Экспорт
	...
	Фреймворк.ПроверитьИстину(СтрНачинаетсяС(Результат.ИнформацияОбОшибке.Описание, "[ какой-то коммит... ]: Недопустимое значение параметра"));
...
Процедура ПередачаДвоичныхДанныхВФонеТриФайла200OkЕстьПараметрыСобытия(Фреймворк) Экспорт
	...
	ОтборЖурналаРегистрацииИнформация = ОтборЖурналаРегистрации("ОбработчикиСобытий.Core.ОтправкаДанныхПолучателю");
	ОтборЖурналаРегистрацииОшибка = ОтборЖурналаРегистрации("ОбработчикиСобытий.Core.ОтправкаДанныхПолучателю", "Ошибка");
	...
	Фреймворк.ПроверитьИстину(СтрНачинаетсяС(МассивФоновыхЗаданий[1].ИнформацияОбОшибке.Описание, "[ какой-то коммит... ]: Недопустимое значение параметра"));
	...
	Фреймворк.ПроверитьВхождение(ЖурналРегистрацииОшибка[0].Комментарий, "В структуре ожидалось свойство Адрес (тип Строка)");	
	Фреймворк.ПроверитьИстину(СтрНачинаетсяС(ЖурналРегистрацииОшибка[0].Комментарий, "[ какой-то коммит... ]: Недопустимое значение параметра"));
