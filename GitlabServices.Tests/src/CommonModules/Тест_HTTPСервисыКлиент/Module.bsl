...
Процедура Тест_WebhooksPOST(Фреймворк) Экспорт
	...
	URL = Фреймворк.ПолучитьСохраненноеЗначениеИзКонтекстаСохраняемого("МестоположениеСервисовИБРаспределителя");
	ЭталонТелоPOSTЗапросаJSON = Фреймворк.ПолучитьСохраненноеЗначениеИзКонтекстаСохраняемого("ЭталонТелоPOSTЗапросаJSON");
	BadURLEpf = URL + "/webhooks/epf3/push";
	BadURLPush = URL + "/webhooks/epf/push3";
	URL = URL + "/webhooks/epf/push";
	ПутьЭталонТелоPOSTЗапросаJSON = Фреймворк.Объект.КаталогПроекта + ЭталонТелоPOSTЗапросаJSON;
	СекретныйТокен = "блаблаблаюниттест";	

	Тест_РаботаСИнтернетСервисамиСервер.УстановитьЗагружатьФайлыИзВнешнегоХранилища(Ложь);
	Тест_HTTPСервисыСервер.УдалитьЭлементыСправочникаОбработчикиСобытий(СекретныйТокен);
	Тест_HTTPСервисыСервер.ДобавитьОбработчикСобытий("ЮнитТест1", СекретныйТокен);
	
	// 403
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(URL, "ФэйковыйСекретныйТокен");
	Фреймворк.ПроверитьРавенство(Ответ.КодСостояния, 403);
	ТелоОтвета = ТелоОтветаКакЧтениеТекста(Ответ);
	Фреймворк.ПроверитьВхождение(ТелоОтвета, "Секретный токен не найден");
	
	// 423
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(URL, СекретныйТокен);
	...
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(URL, СекретныйТокен, , Дополнительно);
	...
	Заголовки.Вставить("X-Gitlab-Token", СекретныйТокен);
	Дополнительно = Новый Структура("Заголовки", Заголовки);
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(BadURLEpf, СекретныйТокен, , Дополнительно);
	Фреймворк.ПроверитьРавенство(Ответ.КодСостояния, 400);
	ТелоОтвета = ТелоОтветаКакЧтениеТекста(Ответ);
	...
	Заголовки.Вставить("X-Gitlab-Token", СекретныйТокен);
	Дополнительно = Новый Структура("Заголовки", Заголовки);
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(BadURLPush, СекретныйТокен, , Дополнительно);
	...
	// 200
	Ответ = Тест_HTTPСервисыСервер.WebhooksPOST(URL, СекретныйТокен, ПутьЭталонТелоPOSTЗапросаJSON);
