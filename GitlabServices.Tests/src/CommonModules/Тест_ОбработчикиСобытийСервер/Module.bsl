...
Процедура ЗагрузитьИсториюСобытий(Фреймворк) Экспорт
	...
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[0].Операция, "Операция");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[1].Операция, "Операция");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[0].Уровень, "Информация");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[1].Уровень, "Информация");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[0].Событие, "ОбработчикиСобытий.Операция.Что.Что");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[1].Событие, "ОбработчикиСобытий.Операция.Что.Что.200");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[0].ПредставлениеСобытия, "Что.Что");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[1].ПредставлениеСобытия, "Что.Что");
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[0].КодСостоянияHTTP, 0);
	Фреймворк.ПроверитьРавенство(ОбработчикСобытия.EventsHistory[1].КодСостоянияHTTP, 200);
...
Процедура СохранитьДанныеЗапросаЗаписьОшибкаЗаписи(Фреймворк) Экспорт
	...
	Попытка
		...
		ВызватьИсключение НСтр("ru = 'Передача пустого обработчика событий должна вызывать ошибку.'");
	Исключение
		// then
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		Фреймворк.ПроверитьВхождение(ИнформацияОбОшибке.Описание, "Ошибка при вызове метода контекста (Записать)");
	КонецПопытки;
