...
Функция WebhooksPOST(URL, СекретныйТокен, ПутьЭталон = Неопределено, ТестовыеДанные = Неопределено) Экспорт

	JSON = Неопределено;
	Если ПутьЭталон <> Неопределено Тогда	
		ЧтениеТекста = Новый ЧтениеТекста(ПутьЭталон, КодировкаТекста.UTF8);
