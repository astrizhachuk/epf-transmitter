...
Процедура ПолучитьФайлыКОтправкеПоДаннымЗапроса(Фреймворк) Экспорт
	...
	// when
	Результат = Gitlab.FilesByQueryData( Справочники.ОбработчикиСобытий.ПустаяСсылка(), ДанныеЗапроса);
...
// 	Фреймворк - ФреймворкТестирования - Фреймворк тестирования
//
Процедура FileActions(Фреймворк) Экспорт

	// when	
	Результат = GitlabCached.FileActions();
	// then
