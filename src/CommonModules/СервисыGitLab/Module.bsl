...
// Ищет и устанавливает в качестве идентификатора элемент справочника Webhook (первый попавшийся),
// в котором секретный ключ (токен) равен секретному токену из заголовка HTTP-запроса.
// Если не найдено, то возвращается пустая ссылка. 
// 
// Параметры:
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP-запрос.
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на элемент справочника или пустая ссылка. 
Процедура ЗаполнитьИдентификаторWebhook(Знач HTTPСервисЗапрос, ИдентификаторWebhook = Неопределено) Экспорт
	
	Перем СекретныйТокен;
	
	Если ИдентификаторWebhook <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	СекретныйТокен = СекретныйТокен(HTTPСервисЗапрос);
	ИдентификаторWebhook = Справочники.Webhooks.НайтиИндентификаторПоСекретномуТокену(СекретныйТокен);
...
Процедура ПроверитьЗапросUpdates(Знач ИдентификаторWebhook,
	...
	Метод = РаботаСИнтернетСервисами.ПроверитьЗначениеПараметра(HTTPСервисЗапрос, "ИмяМетода", "updates");
