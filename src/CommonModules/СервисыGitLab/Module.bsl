////////////////////////////////////////////////////////////////////////////////
// Работа с сервисами взаимодействия 1C и GitLab.
//  
////////////////////////////////////////////////////////////////////////////////
#Область ПрограммныйИнтерфейс

Процедура ЗапуститьОбработкуДанныхВФоне(Знач ИдентификаторWebhook, Знач ДанныеТелаЗапроса, Знач ЭтоРучнойЗапуск = Ложь) Экспорт
	...
	Если ТипЗнч(ДанныеТелаЗапроса) <> Тип("Соответствие") Тогда
		...
		СервисыGitLab.ЗалогироватьОшибку("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
	...
	Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
		...
		СервисыGitLab.ЗалогироватьПредупреждение("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
	...
	СервисыGitLab.ЗалогироватьИнформацию("Core", "ОбработкаДанныхВФоне.Начало", ИдентификаторWebhook, ТекстСообщения);
	
КонецПроцедуры

// TODO экспортная? или прячем?
...
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
...
Процедура ПередатьДвоичныеДанныеВИБПриемник(Знач ИдентификаторWebhook, Знач КлючЗапросаGitLab, Знач ИмяФайла, Знач ДвоичныеДанные, Знач ПараметрыДоставки) Экспорт
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон(ТекстСообщения, КлючЗапросаGitLab, ИмяФайла, ПараметрыДоставки.МестоположениеСервиса, ИнформацияОбОшибке().Описание);
		СервисыGitLab.ЗалогироватьОшибку("Core", "ПередачаФайлаВИБПриемник", ИдентификаторWebhook, ТекстСообщения);
		
		// Исключений не пробрасываем, так как процедура в асинхронном режиме и должна выполниться.
		Возврат;
		
	КонецПопытки;
	
	ТекстСообщения = НСтр("ru = '%1: Доставлено: %2; Файл: %3; Адрес доставки: %4'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, КлючЗапросаGitLab, Строка(Результат), ИмяФайла, ПараметрыДоставки.МестоположениеСервиса);
	СервисыGitLab.ЗалогироватьИнформацию("Core",
...
// Получает тело HTTP запроса в формате json и преобразует его в Соответствие.
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP-запрос. 
// 	HTTPСервисОтвет - HTTPСервисОтвет - HTTP-ответ.
// 	ДанныеТелаЗапроса - Неопределено, Соответствие - исходящий параметр,
//		тело запроса преобразованное в Соответствие json.
//		Исходный текст тела добавляется в структуру с ключом "json".
Процедура ПолучитьДанныеТелаЗапроса(Знач ИдентификаторWebhook,
									Знач HTTPСервисЗапрос,
										 HTTPСервисОтвет,
										 ДанныеТелаЗапроса = Неопределено) Экспорт
	
	Перем Поток;
	Перем ЧтениеJSON;
	Перем ИменаСвойствСоЗначениямиДата;
	Перем ПрочитатьВСоответствие;
	
	СервисыGitLab.ЗалогироватьИнформацию("GitLab",
										 "ОбработкаЗапроса.Начало",
										 ИдентификаторWebhook,
										 НСтр("ru = 'Начало получения тела запроса...'"));
	
	Поток = HTTPСервисЗапрос.ПолучитьТелоКакПоток();
	...
	Попытка
		...
		ИменаСвойствСоЗначениямиДата.Добавить("timestamp");
		ПрочитатьВСоответствие = Истина;
		ДанныеТелаЗапроса      = ПрочитатьJSON(ЧтениеJSON, ПрочитатьВСоответствие, ИменаСвойствСоЗначениямиДата);
		
		ЧтениеJSON.Закрыть();
		СервисыGitLab.ЗалогироватьИнформацию("GitLab",
											 "ОбработкаЗапроса.Окончание",
											 ИдентификаторWebhook,
											 НСтр("ru = 'Окончание получения тела запроса...'"));
		
		РаботаСИнтернетСервисами.ДополнитьДанныеИсходнымТекстомJSON(Поток, "json", ДанныеТелаЗапроса);
	...
	Исключение
		...
		СервисыGitLab.ЗалогироватьОшибку("GitLab",
										 "ОбработкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = '" + ИнформацияОбОшибке().Описание + "'"));
...
// Поиск обработчика событий по секретному токену (ключу), определяемому из заголовков HTTP-запроса.
// 
// Параметры:
// 	HTTPСервисЗапрос - HTTPСервисЗапрос, HTTPЗапрос - HTTP-запрос.
// Возвращаемое значение:
// 	Неопределено, СправочникСсылка.Webhooks - ссылка на элемент справочника обработчиков событий;
//		если обработчик не найден, то возвращается Неопределено.
Функция НайтиОбработчикСобытияПоHTTPЗапросу(Знач HTTPСервисЗапрос) Экспорт
	
	Перем СекретныйТокен;
	Перем ОбработчикСобытий;

	ОбработчикСобытий = Неопределено;	
	Если НЕ (ТипЗнч(HTTPСервисЗапрос) = Тип("HTTPСервисЗапрос") ИЛИ ТипЗнч(HTTPСервисЗапрос) = Тип("HTTPЗапрос")) Тогда
		Возврат ОбработчикСобытий;						
	КонецЕсли;
	
	СекретныйТокен = СекретныйТокен(HTTPСервисЗапрос);
	
	Если СекретныйТокен = Неопределено Тогда
		Возврат ОбработчикСобытий;
	КонецЕсли;
	
	ОбработчикСобытий = Справочники.Webhooks.НайтиПоСекретномуТокену(СекретныйТокен);
	
	Возврат ОбработчикСобытий; 
	
КонецФункции

// Сохраняет исходные данные HTTP-запроса (См. РегистрСведений.Webhooks).
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
...
// Сохраняет двоичные данные внешних файлов с их описанием (См. РегистрСведений.Webhooks).
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
// 	Ключ - Строка - значение checkout_sha коммита;
...
// * МестоположениеСервисаИБПолучателя - Строка - (См. Константа.МестоположениеСервисаИБПолучателя);
// * ТаймаутИБПолучателя - Число - (См. Константа.ТаймаутИБПолучателя);
// * ТаймаутGitLab - Число - (См. Константа.ТаймаутGitLab);
// * ПарольПользователяИБПолучателя - Строка - (См. Константа.ПарольПользователяИБПолучателяGitLab);
// * ПользовательИБПолучателя - Строка - (См. Константа.ПользовательИБПолучателяGitLab);
...
Функция НастройкиСервисовGitLab() Экспорт
	...
	Настройки.Вставить("ПользовательИБПолучателя", "");
	Настройки.Вставить("ПарольПользователяИБПолучателя", "");
	...
	Попытка
		...
		Настройки.ПользовательИБПолучателя          = ПользовательИБПолучателя();
		Настройки.ПарольПользователяИБПолучателя    = ПарольПользователяИБПолучателя();
	...
	Исключение
		...
		ЗалогироватьОшибку("System", "Настройки", , ТекстСообщения);
		ВызватьИсключение;
	КонецПопытки;
...
// 	ИдентификаторWebhook - СправочникиСсылка.Webhooks - ссылка на логируемый объект;
...
Процедура ЗалогироватьИнформацию(Знач Источник,
								 Знач Событие,
								 Знач ИдентификаторWebhook,
								 Знач ТекстСообщения,
									  HTTPСервисОтвет = Неопределено) Экспорт
	
	//ЗалогироватьИнформациюПроверкаВходящихПараметров(Событие, ИдентификаторWebhook, ТекстСообщения);
...
// 	HTTPСервисОтвет - Неопределено, HTTPСервисОтвет - если передан объект HTTPСервисОтвет, то в него будет
// 		добавлено тело сообщения в формате JSON.
Процедура ЗалогироватьПредупреждение(Знач Источник,
									 Знач Событие,
									 Знач ИдентификаторWebhook,
									 Знач ТекстСообщения,
										  HTTPСервисОтвет = Неопределено) Экспорт
	
	//ЗалогироватьПредупреждениеПроверкаВходящихПараметров(Событие, ИдентификаторWebhook, ТекстСообщения);
	ЛогированиеЯдро(Источник,
...
// 	HTTPСервисОтвет - Неопределено, HTTPСервисОтвет - если передан объект HTTPСервисОтвет, то в него будет
// 		добавлено тело сообщения в формате json.
Процедура ЗалогироватьОшибку(Знач Источник,
							 Знач Событие,
							 Знач ИдентификаторWebhook = Неопределено,
							 Знач ТекстСообщения,
								  HTTPСервисОтвет = Неопределено) Экспорт
	
	//ЗалогироватьОшибкуПроверкаВходящихПараметров(Событие, ИдентификаторWebhook, ТекстСообщения);
	ЛогированиеЯдро(Источник,
...
Процедура ДекораторЗагрузитьИсториюСобытий(Знач Метаданные, Знач ТабличнаяЧасть = Неопределено, Знач ПараметрыОтбора, ДобавленоЗаписей) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСправочник(Метаданные) Тогда
		...
		Если ПараметрыОтбора.Свойство("Данные", Ссылка) Тогда
			...
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Для Справочник.%1 не реализован метод ЗагрузитьИсториюСобытий().'"), Метаданные.Имя);
				СервисыGitLab.ЗалогироватьОшибку("System", "ИсторияСобытий", Ссылка, ТекстСообщения);
				ВызватьИсключение;
			КонецПопытки
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьЗапросНаMergeRequests(Знач ДанныеТелаЗапроса) Экспорт
	...
	ИмяСервера = ПараметрыПроекта.СтруктураURI.ИмяСервера;
...
Процедура УдалитьПользовательскиеНастройкиВДанныхКоммита(Знач ДанныеТелаЗапроса, Знач ИдентификаторКоммита, НастройкиПоУмолчанию = "") Экспорт
...
Процедура ДополнитьДанныеТелаЗапросаПользовательскимиНастройками(Знач ИдентификаторКоммита, Знач JSON, ДанныеТелаЗапроса) Экспорт
	...
	РаботаСИнтернетСервисами.ПреобразоватьПотокИзФорматаJSONВКоллекцию(Поток, Истина, ПользовательскиеНастройки);
...
// 
// Параметры:
// 	КлючЗаписи - РегистрСведенийКлючЗаписи.Webhooks - ключ записи с сохраненными записями;
...
Функция ИсходныеДанныеЗапросаВФорматеJSON(Знач КлючЗаписи, Знач Действие) Экспорт
	...
	ДанныеТелаЗапроса = РегистрыСведений.Webhooks.ПолучитьДанныеТелаЗапроса(КлючЗаписи);
...
// Проверяет преобразованные в Соответствие данные тела запроса на наличие необходимых данных.
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
// 	ДанныеТелаЗапроса - Соответствие - преобразованные в Соответствие данные HTTP-запроса;
// 	HTTPСервисОтвет - HTTPСервисОтвет - HTTP-ответ;
Процедура ПроверитьТелоЗапроса(Знач ИдентификаторWebhook, Знач ДанныеТелаЗапроса, HTTPСервисОтвет) Экспорт
	
	//TODO отрефакторить! найти или создать нормальный способ проверки структуры на наличие обязательных свойств. Текущая реализация на скорую руку!
	//посмотреть в сторону этого:
	//ЭлементыДляПроверки = Новый Массив;
	//ЭлементыДляПроверки.Добавить("project");
	//ЭлементыДляПроверки.Добавить("project/web_url");
	////ЭлементыДляПроверки.Добавить("project/path_with_namespace");
	//ЭлементыДляПроверки.Добавить("project/id");
	//ЭлементыДляПроверки.Добавить("commits");
	//ЭлементыДляПроверки.Добавить("commits/id");
	//ОбщегоНазначенияКлиентСервер.Проверить(Проект <> Неопределено,
	//НСтр("ru = 'В данных тела запроса отсутствует элемент project'"),
	//"СервисыGitLab.ДополнитьДанныеЗапросаНастройками");
	
	СервисыGitLab.ЗалогироватьИнформацию("GitLab",
										 "ПроверкаЗапроса.Начало",
										 ИдентификаторWebhook,
										 НСтр("ru = 'Начало проверки тела запроса...'"));
	...
	Если ДанныеТелаЗапроса = Неопределено Тогда
		...
		СервисыGitLab.ЗалогироватьОшибку("GitLab"
										 "ПроверкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'Отсутствует тело запроса.'"),
										 HTTPСервисОтвет);
		Возврат;		
	КонецЕсли;
	
	Если ДанныеТелаЗапроса.Получить("checkout_sha") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		СервисыGitLab.ЗалогироватьОшибку("GitLab",
										 "ПроверкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'В данных запроса отсутствует элемент checkout_sha.'"),
										 HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	
	project = ДанныеТелаЗапроса.Получить("project");
	Если project = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		СервисыGitLab.ЗалогироватьОшибку("GitLab",
										 "ПроверкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'В данных запроса отсутствует элемент project.'"),
										 HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	
	Если project.Получить("web_url") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		СервисыGitLab.ЗалогироватьОшибку("GitLab",
										 "ПроверкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'В данных запроса отсутствует элемент project/web_url.'"),
										 HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	
	//Если project.Получить("path_with_namespace") = Неопределено Тогда
	//    HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
	//	RESTСервисыСервер.ДобавитьВТелоОтветаСообщениеОбОшибке(HTTPСервисОтвет, "В данных запроса отсутствует элемент project/path_with_namespace.");
	//	Возврат;
	//КонецЕсли;
	
	Если project.Получить("id") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		СервисыGitLab.ЗалогироватьОшибку("GitLab",
										 "ПроверкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'В данных запроса отсутствует элемент project/id.'"),
										 HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	
	commits = ДанныеТелаЗапроса.Получить("commits");
	Если commits = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		СервисыGitLab.ЗалогироватьОшибку("ПроверкаЗапроса",
										 "ОбработкаЗапроса",
										 ИдентификаторWebhook,
										 НСтр("ru = 'В данных запроса отсутствует элемент commits.'"),
										 HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	
	Для каждого commit Из commits Цикл
		Если commit.Получить("id") = Неопределено Тогда
			HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
			СервисыGitLab.ЗалогироватьОшибку("GitLab",
											 "ПроверкаЗапроса",
											 ИдентификаторWebhook,
											 НСтр("ru = 'В данных запроса отсутствует элемент commits/../id.'"),
											 HTTPСервисОтвет);
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	СервисыGitLab.ЗалогироватьИнформацию("GitLab",
										 "ПроверкаЗапроса.Окончание",
										 ИдентификаторWebhook,
										 НСтр("ru = 'Окончание проверки тела запроса...'"));
	
КонецПроцедуры

// Проверка авторизации и корректность заголовков для обработки события "Push Hook".
// 
// Параметры:
//	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP запрос;
// 	HTTPСервисОтвет - HTTPСервисОтвет - HTTP ответ результата проверки;
Процедура ПроверитьЗапросUpdates(Знач ИдентификаторWebhook,
								 Знач HTTPСервисЗапрос,
									  HTTPСервисОтвет = Неопределено) Экспорт
	...
	// Пока поддерживается только обработка изменений внешних отчетов и обработок.
	Если НЕ ЭтоРепозиторийВнешнихОтчетовИОбработок(HTTPСервисЗапрос) Тогда
		
		СервисыGitLab.ЗалогироватьПредупреждение("GitLab",
												 "ОбработкаЗапроса",
												 ИдентификаторWebhook,
												 НСтр("ru = 'Сервис доступен только для внешних отчетов и обработок.'"),
												 HTTPСервисОтвет);
	...
	isUpdates = РаботаСИнтернетСервисами.ЗначениеВКоллекцииСуществует(HTTPСервисЗапрос.ПараметрыURL,
						"ИмяМетода", "updates");
	
	Если НЕ isUpdates Тогда
		
		СервисыGitLab.ЗалогироватьПредупреждение("GitLab",
												 "ОбработкаЗапроса",
												 ИдентификаторWebhook,
												 НСтр("ru = 'Не определен метод сервиса, см. описание сервиса ../gitlab'"),
												 HTTPСервисОтвет);
	...
	Если НЕ ЭтоPush(HTTPСервисЗапрос) Тогда
		
		СервисыGitLab.ЗалогироватьПредупреждение("GitLab",
												 "ОбработкаЗапроса",
												 ИдентификаторWebhook,
												 НСтр("ru = 'Сервис обрабатывает только события ""Push Hook"".'"),
												 HTTPСервисОтвет);
...
// Возвращаемое значение:
// 	Структура - Описание:
// * МестоположениеСервиса - Строка - реальное местоположение сервиса. Если не заполнено, то будет взято
// 		местоположение сервиса в момент загрузки его описания в WS-ссылки.
// * Таймаут - Число - таймаут запроса, в секундах, если 0, то не установлен;
// * Пользователь - Строка - имя пользователя в ИБ-приемнике с ролью, позволяющей использовать ws для
//		загрузки внешних отчетов/обработок. 
...
Функция ИнициализироватьПараметрыДоставки() Экспорт
	...
	ПараметрыДоставки.Вставить("МестоположениеСервиса", МестоположениеСервисаИБПолучателя());
	ПараметрыДоставки.Вставить("Таймаут", 0);
	ПараметрыДоставки.Вставить("Пользователь", "");
	ПараметрыДоставки.Вставить("Пароль", "");
...
// Получает значение заголовка "X-Gitlab-Event" из HTTP запроса.
// 
// Параметры:
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP запрос.
// Возвращаемое значение:
// 	Строка, Неопределено - значение заголовка "X-Gitlab-Event" из запроса.
Функция GitLabEvent(Знач HTTPСервисЗапрос)
	
	Перем Результат;
	
	Результат = HTTPСервисЗапрос.Заголовки.Получить("X-Gitlab-Event");
	Возврат Результат;
...
Функция ЭтоPush(Знач HTTPСервисЗапрос)
	...
	Событие = GitLabEvent(HTTPСервисЗапрос);
	
	Если ((Событие <> Неопределено) И (Событие = "Push Hook")) Тогда
		Результат = Истина;
	КонецЕсли;
...
// 	ДанныеТелаЗапроса - (См. ПолучитьДанныеТелаЗапроса.ДанныеТелаЗапроса);
Процедура ДополнитьДанныеЗапросаНастройками(Знач ИдентификаторWebhook, Знач ПараметрыПроекта, Знач Таймаут = 0, ДанныеТелаЗапроса)
	...
	НастройкиМаршрутизации = НастройкиМаршрутизации(ИдентификаторWebhook, ОписаниеФайловНастроек, ПараметрыПроекта, PrivateToken, Таймаут);
...
//  	** ???         - Строка - схема из URI.
//   	** ???         - Строка - логин из URI.
Функция НастройкиМаршрутизации(Знач ИдентификаторWebhook, Знач ОписаниеНастроек, Знач ПараметрыПроекта, Знач PrivateTokenGitLab, Знач Таймаут = 0)
	...
	Для каждого ОписаниеФайла Из ОписаниеНастроек Цикл
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			РаботаСИнтернетСервисами.ПреобразоватьТелоHTTPОтветаИзФорматаJSONВСоответствие(HTTPОтвет, НастройкиИзФайла);
		КонецЕсли;
...
// Получает секретный токен webhook из заголовка HTTP-запроса.
// 
// Параметры:
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP запрос. 
// Возвращаемое значение:
// 	Неопределено, Строка - значение секретного токена для webhook.
Функция СекретныйТокен(Знач HTTPСервисЗапрос)
	Возврат HTTPСервисЗапрос.Заголовки.Получить("X-Gitlab-Token");
КонецФункции

// Получает значение константы с именем пользователя в ИБ-получателях, под которым происходит подключение к сервисам
// конечных точек доставки файлов.
// 
// Параметры:
// Возвращаемое значение:
// 	Строка - возвращает имя пользователя в ИБ-получателях (макс. 50), при незаполненном значении вызывается исключение;
Функция ПользовательИБПолучателя()
	
	Перем ПользовательИБПолучателя;
	
	ПользовательИБПолучателя = Константы.ПользовательИБПолучателяGitLab.Получить();
	ОбщегоНазначенияКлиентСервер.Проверить(НЕ ПустаяСтрока(ПользовательИБПолучателя),
		НСтр("ru = 'Настройка ""Пользователь"" для ИБ-получателей должна быть заполнена.'"),
		"СервисыGitLab.ПользовательИБПолучателя");
	
	Возврат ПользовательИБПолучателя;
	
КонецФункции

// Получает значение константы с паролем пользователя в ИБ-получателях, под которым происходит подключение к сервисам
// конечных точек доставки файлов.
// 
// Параметры:
// Возвращаемое значение:
// 	Строка - возвращает пароль пользователя в ИБ-получателях (макс. 50); 
Функция ПарольПользователяИБПолучателя()
	
	Перем ПарольПользователяИБПолучателя;
	
	ПарольПользователяИБПолучателя = Константы.ПарольПользователяИБПолучателяGitLab.Получить();
	
	Возврат ПарольПользователяИБПолучателя;
...
// Местоположение сервиса получения файлов в ИБ-получателе по умолчанию (для wsdl).
// 
// Параметры:
// Возвращаемое значение:
// 	Строка - адрес местоположения сервиса (без аргумента ?wsdl);
Функция МестоположениеСервисаИБПолучателя()
	
	Перем МестоположениеСервиса;
	
	МестоположениеСервиса          = Константы.МестоположениеСервисаИБПолучателя.Получить();
	МестоположениеСервисаЗаполнено = НЕ (МестоположениеСервиса = Неопределено ИЛИ ПустаяСтрока(МестоположениеСервиса));
	МестоположениеСервиса          = ?(МестоположениеСервисаЗаполнено, МестоположениеСервиса, СтрРазделить(Метаданные.WSСсылки.EpfService.URLИсточника, "?")[0]);
	
	Возврат МестоположениеСервиса;
...
Процедура ЛогированиеЯдро(Знач Источник = "System", Знач Тип, Знач Событие, Знач Идентификатор = Неопределено, Знач ТекстСообщения, ОтветНаЗапрос)
	
	Перем ИмяСобытия;
	Перем ЭтоHTTPСервисОтвет;
	
	ИмяСобытия = СтрШаблон("%1.%2.%3", "Webhooks", Источник, Событие);
	...
	Если ЭтоHTTPСервисОтвет ИЛИ ЭтоHTTPОтвет Тогда
		...
		Если ЭтоHTTPСервисОтвет Тогда
			Если Тип = УровеньЖурналаРегистрации.Информация Тогда
				РаботаСИнтернетСервисами.СобытиеЖурналаРегистрацииВТелоОтвета(ОтветНаЗапрос, ТекстСообщения, Перечисления.УровеньЖурналаРегистрации.Информация);
			Иначе
				РаботаСИнтернетСервисами.СобытиеЖурналаРегистрацииВТелоОтвета(ОтветНаЗапрос, ТекстСообщения, Перечисления.УровеньЖурналаРегистрации.Ошибка);			
			КонецЕсли;
		КонецЕсли;
	...
	ЗаписьЖурналаРегистрации(ИмяСобытия,
							 Тип,
							 Метаданные.Справочники.Webhooks,
							 Идентификатор,
							 ТекстСообщения);
	
КонецПроцедуры

// Сохраняет "исходные данные HTTP-запроса" или "обрабатываемые файлы" (См. РегистрСведений.Webhooks).
// 
// Параметры:
// 	ИдентификаторWebhook - СправочникСсылка.Webhooks - ссылка на описание webhook;
// 	Ключ - Строка - значение checkout_sha коммита;
...
Процедура СохранитьДанныеВИБ(Знач ИдентификаторWebhook, Знач Ключ, Знач Ресурс, Знач Данные)
	...
	ЭлементБлокировки       = Блокировка.Добавить("РегистрСведений.Webhooks");
	...
	Попытка
		...
		КлючЗаписи = РегистрыСведений.Webhooks.СоздатьКлючЗаписи(ПараметрыКлюча);
		ЗаписываемыеДанные = Новый Структура(Ресурс, Новый ХранилищеЗначения(Данные));
		РегистрыСведений.Webhooks.ЗаписатьЗначенияРесурсов(КлючЗаписи, ЗаписываемыеДанные); 
	...
	Исключение
		...
		СервисыGitLab.ЗалогироватьПредупреждение("Core", "СохранениеДанных",
										 ИдентификаторWebhook, ТекстСообщения);
...
Процедура ФоноваяОбработкаДанныхЗапроса(Знач ИдентификаторWebhook,
	...
	Попытка
		...
		Если ДанныеДляОтправки.Количество() = 0 Тогда
			...
			ТекстСообщения = СтрШаблон(ТекстСообщения, КлючЗапросаGitLab);
			СервисыGitLab.ЗалогироватьИнформацию("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
		...
		ПараметрыДоставки.Пароль       = ПарольПользователяИБПолучателя();
		ПараметрыДоставки.Таймаут      = ТаймаутИБПолучателя();
		
		Для каждого ТочкаДоставки Из ДанныеДляОтправки Цикл
			
			ПараметрыДоставки.МестоположениеСервиса = ТочкаДоставки.Адрес;
			...
			Попытка
				...
				Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
					...
					СервисыGitLab.ЗалогироватьПредупреждение("Core", "ПередачаФайлаВИБПриемник", ИдентификаторWebhook, ТекстСообщения);
					
					Продолжить;
					
				КонецЕсли;
				
				ЗаданиеОтправкаФайла = ФоновыеЗадания.Выполнить("СервисыGitLab.ПередатьДвоичныеДанныеВИБПриемник",
																ПараметрыЗадания,
																КлючОтправкиФайла,
																"СервисыGitLab: " + КлючЗапросаGitLab);
			Исключение
		...
		СервисыGitLab.ЗалогироватьИнформацию("Core", "ОбработкаДанныхВФоне.Окончание", ИдентификаторWebhook, ТекстСообщения);
...
Функция ВыполнитьHTTPЗапросНаПолучениеФайла(Знач ИдентификаторWebhook, Знач СтруктураURI, Знач ПутьКФайлу, Знач PrivateTokenGitLab, Знач Таймаут = 0) Экспорт
	...
	Исключение
		...
		СервисыGitLab.ЗалогироватьОшибку("Core", "ПолучениеДанныхДляОтправкиФайлов",
								 ИдентификаторWebhook,
								 СтрШаблон(ТекстСообщения, СтруктураURI.ИмяСервера, ПутьКФайлу));
	    //ВызватьИсключение;
	КонецПопытки;
...
Функция ПодготовитьДанныеКОтправке(Знач ИдентификаторWebhook, Знач Параметры)
	...
	СервисыGitLab.ЗалогироватьИнформацию("Core", "ПодготовкаДанных.Начало", ИдентификаторWebhook, ТекстСообщения);

	Если Параметры.ЭтоРучнойЗапуск Тогда
		КлючЗапроса = РегистрыСведений.Webhooks.СоздатьКлючЗаписи(Новый Структура("Идентификатор, Ключ", ИдентификаторWebhook, Параметры.КлючЗапросаGitLab));
		ДанныеДляОтправки = РегистрыСведений.Webhooks.ПолучитьДанныеОтправки(КлючЗапроса);
	Иначе
		ДополнитьДанныеЗапросаНастройками(ИдентификаторWebhook, Параметры.ПараметрыПроектаРепозитория, Таймаут, ДанныеТелаЗапроса);
	...
	ТекстСообщения = НСтр("ru = '%1: подготовка данных к отправке.'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Параметры.КлючЗапросаGitLab);
	СервисыGitLab.ЗалогироватьИнформацию("Core", "ПодготовкаДанных.Окончание", ИдентификаторWebhook, ТекстСообщения);
...
Функция ВсеДействияНадДвоичнымиФайламиВнешнегоХранилища(Знач ДанныеТелаЗапроса, Знач ПараметрыПроекта)
	...
	Для каждого Коммит Из Коммиты Цикл
		...
		Для каждого ВидОперации Из ВидыОперацийНадФайлами Цикл
			...
			ДополнитьТаблицуОтправкиДанныхИзменениямиРепозитория(ПолныеИменаФайлов, ВидОперации, ПараметрыЗаполнения, ТаблицаИзменений);
...
Функция ДанныеДляОтправки(Знач ИдентификаторWebhook, Знач ДанныеТелаЗапроса, Знач ПараметрыПроекта, Знач Таймаут = 0)
	...
	Для каждого Коммит Из Коммиты Цикл
		...
		Идентификатор     = Коммит.Получить("id");
		ДанныеДляОтправки = ПолучитьДанныеДляОтправкиССервераGitLab(ИдентификаторWebhook, ПараметрыПроекта, ИзмененияВРепозитории, Идентификатор, PrivateToken, Таймаут);
...
Функция ПолучитьДанныеДляОтправкиССервераGitLab(Знач ИдентификаторWebhook,
	...
	Для каждого ОписаниеФайла Из ДанныеДляОтправки Цикл
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			...
			ОписаниеФайла.ИмяФайла = ПерекодироватьСтроку(ИмяФайла, "windows-1251");
			...
			СервисыGitLab.ЗалогироватьИнформацию("Core",
												 "ПолучениеДанныхДляОтправкиФайлов",
												 ИдентификаторWebhook,
												 ТекстСообщения,
												 HTTPОтвет);
		...
		Иначе
			...
			СервисыGitLab.ЗалогироватьОшибку("Core",
											 "ПолучениеДанныхДляОтправкиФайлов",
											 ИдентификаторWebhook,
											 ТекстСообщения,
											 HTTPОтвет);
...
// * ПутьКФайлу - Строка - относительный путь к файлу в формате URL
// * Операция - Строка - вид операции над файлом (A - добавлен, D - удален, M - изменен).
Процедура ДополнитьТаблицуОтправкиДанныхИзменениямиРепозитория(Знач ИменаФайлов, Знач ВидОперации, Знач Параметры, ТаблицаИзменений)
...
// Проверят запрос на наличие параметра ТипВнешнегоХранилища и равенство его "epf",
// что соответствует типу хранилища для внешних отчетов и обработок. 
// 
// Параметры:
// 	HTTPСервисЗапрос - HTTPСервисЗапрос - HTTP запрос.
// Возвращаемое значение:
// 	Булево - Истина, если это репозиторий для внешних отчетов и обработок, иначе - Ложь.
Функция ЭтоРепозиторийВнешнихОтчетовИОбработок(Знач HTTPСервисЗапрос)
	
	Перем ТипВнешнегоХранилища;
	
	ТипВнешнегоХранилища = HTTPСервисЗапрос.ПараметрыURL.Получить("ТипВнешнегоХранилища");
	Возврат (ТипВнешнегоХранилища <> Неопределено И ТипВнешнегоХранилища = "epf");
	
КонецФункции

Процедура ПередатьДвоичныеДанныеВИБПриемникПроверкаВходящихПараметров(Знач ИмяФайла, Знач ДвоичныеДанные, Знач ПараметрыДоставки)
	...
	ПараметрыДоставки,
	Тип("Структура"),
	Новый Структура("МестоположениеСервиса, Пользователь, Пароль", Тип("Строка"), Тип("Строка"), Тип("Строка")));
	
КонецПроцедуры

//Процедура ЗалогироватьИнформациюПроверкаВходящихПараметров(Знач Событие, Знач Идентификатор, Знач ТекстСообщения)
//	
//	ЛогированиеПроверкаВходящихПараметров("ЗалогироватьИнформацию", Событие, Идентификатор, ТекстСообщения);
//	
//КонецПроцедуры
//
//Процедура ЗалогироватьПредупреждениеПроверкаВходящихПараметров(Знач Событие, Знач Идентификатор, Знач ТекстСообщения)
//	
//	ЛогированиеПроверкаВходящихПараметров("ЗалогироватьПредупреждение", Событие, Идентификатор, ТекстСообщения);
//	
//КонецПроцедуры
//
//Процедура ЗалогироватьОшибкуПроверкаВходящихПараметров(Знач Событие, Знач Идентификатор, Знач ТекстСообщения)
//	
//	ЛогированиеПроверкаВходящихПараметров("ЗалогироватьОшибку", Событие, Идентификатор, ТекстСообщения);
//	
//КонецПроцедуры
//
//Процедура ЛогированиеПроверкаВходящихПараметров(Знач ИмяПроцедуры, Знач Событие, Знач Идентификатор, Знач ТекстСообщения)
//													
//	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр(
//		"СервисыGitLab." + ИмяПроцедуры,
//		"Событие",
//		Событие,
//		Тип("Строка"));
//		
...
// 
// Параметры:
// 	Строка - Строка - строка, кодировку которой необходимо преобразовать.
// 	КодировкаИз - Строка - изначальная кодировка;
// 	КодировкаВ - Строка - результирующая кодировка;
// Возвращаемое значение:
// 	Строка - результирующая строка в новой кодировке.
Функция ПерекодироватьСтроку(Знач Строка, Знач КодировкаИз, Знач КодировкаВ = "UTF-8")
