...
Процедура ПолучитьДанныеТелаЗапроса(Знач ИдентификаторWebhook,
	...
	Попытка
		...
		ОбщегоНазначения.ДополнитьКоллекциюТекстомИзПотока(Поток, "json", ДанныеТелаЗапроса);
...
Процедура ДополнитьДанныеТелаЗапросаПользовательскимиНастройками(Знач ИдентификаторКоммита, Знач JSON, ДанныеТелаЗапроса) Экспорт
	...
	ОбщегоНазначения.ПреобразоватьПотокВКоллекциюКакJSON(Поток, Истина, Истина, ПользовательскиеНастройки);
...
Процедура ПроверитьЗапросUpdates(Знач ИдентификаторWebhook,
	...
	isUpdates = ОбщегоНазначения.ЗначениеВКоллекцииСуществует(HTTPСервисЗапрос.ПараметрыURL,
						"ИмяМетода", "updates");
