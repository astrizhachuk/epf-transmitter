...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаписьСуществует = НЕ (КлючЗаписи = Неопределено
		ИЛИ ТипЗнч(КлючЗаписи) <> Тип("РегистрСведенийКлючЗаписи.Webhooks")
...
Функция ЕстьПользовательскиеНастройки(Знач Параметры)
	...
	ДанныеТелаЗапроса = РегистрыСведений.Webhooks.ПолучитьДанныеТелаЗапроса(Параметры.Ключ);
	Если ДанныеТелаЗапроса = Неопределено Тогда
		Возврат Результат;
	КонецЕсли;

	Результат = СервисыGitLab.ЕстьПользовательскиеНастройкиВДанныхКоммита(ДанныеТелаЗапроса, Параметры.СохраняемыеДанные.Коммит);
...
Функция УдалитьПользовательскиеНастройки(Знач Параметры)
	...
	Если НЕ ПустаяСтрока(Результат) Тогда
		ЗаписываемыеДанные = Новый Структура("ДанныеТелаЗапроса", Новый ХранилищеЗначения(ДанныеТелаЗапроса));
		РегистрыСведений.Webhooks.ЗаписатьЗначенияРесурсов(Параметры.Ключ, ЗаписываемыеДанные); 
	КонецЕсли;
...
Процедура СохранитьJSONНаСервере(Знач Параметры)
	...
	СервисыGitLab.ДополнитьДанныеТелаЗапросаПользовательскимиНастройками(Параметры.СохраняемыеДанные.Коммит, Параметры.СохраняемыеДанные.JSON, ДанныеТелаЗапроса);
	Данные = Новый Структура("ДанныеТелаЗапроса", Новый ХранилищеЗначения(ДанныеТелаЗапроса));
	РегистрыСведений.Webhooks.ЗаписатьЗначенияРесурсов(Параметры.Ключ, Данные); 
