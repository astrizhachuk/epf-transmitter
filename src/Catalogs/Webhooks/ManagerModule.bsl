...
// Ищет элемент справочника Webhook (первый попавшийся), в котором секретный токен равен переданному в функцию значению.
// Если не найдено, то возвращается пустая ссылка. 
// 
// Параметры:
// 	СекретныйТокен - ЗаписьЖурналаРегистрации - значение секретного токена (ключа).
// Возвращаемое значение:
// 	СправочникСсылка.Webhooks - ссылка на описание webhook, если не найдено, то возвращается пустая ссылка. 
Функция НайтиИндентификаторПоСекретномуТокену(Знач СекретныйТокен) Экспорт
	
	Перем Запрос;
	Перем РезультатЗапроса;
	Перем Результат;
	
	Результат = Справочники.Webhooks.ПустаяСсылка();
