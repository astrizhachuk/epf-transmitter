...
&AtClient
Процедура DoAfterLoadEventsHistory( Value, Params ) Export
	...
	AddedRecords = 0;	
	ЗагрузитьИсториюСобытийЗавершениеНаСервере( Value, Params, AddedRecords );
	ПоказатьПредупреждение( , НСтр("ru = 'Добавлено записей: '") + Строка(AddedRecords),
			TIMEOUT, НСтр("ru = 'Загрузка истории из журнала регистрации'"));
...
Procedure LoadEventsHistory( Command )
	...
	OpenForm("Справочник.ОбработчикиСобытий.Форма.УсловияОтбора",
				 ,
				 ThisObject,
				 ,
				 ,
