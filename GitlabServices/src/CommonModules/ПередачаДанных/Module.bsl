...
// TODO указать пралильный путь до ИнициализироватьПараметрыДоставки в описании функции
...
Процедура Отправить( Знач ИмяФайла, Знач Данные, Знач ПараметрыДоставки ) Экспорт
	...
	Ответ = КоннекторHTTP.Post( ПараметрыДоставки.ТочкаДоставки, Данные, ДополнительныеПараметры );
	...
	Сообщение.Текст = СтрШаблон( Шаблон, ПараметрыДоставки.ТочкаДоставки, ИмяФайла, ТекстТелаОтвета );
	Сообщение.Сообщить();								
...
Процедура ОтправитьФайлПроверкаВходящихПараметров( Знач ИмяФайла, Знач Данные, Знач ПараметрыДоставки )
	...
	СтруктураПараметров = Новый Структура( "ТочкаДоставки, Token", Тип("Строка"), Тип("Строка") );
	ОбщегоНазначенияКлиентСервер.ПроверитьПараметр( "ПередачаДанных.Отправить",
													"ПараметрыДоставки",
													ПараметрыДоставки,
													Тип("Структура"),
