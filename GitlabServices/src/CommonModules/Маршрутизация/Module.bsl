...
Функция РаспределитьОтправляемыеДанныеПоМаршрутам( Знач ОтправляемыеДанные, Знач ДанныеЗапроса ) Экспорт
	...
	Перем МаршрутыОтправкиФайлов;
	Перем ОтправляемыйФайл;
	Перем CommitSHA;
	...
	Для Каждого ОписаниеФайла Из ОтправляемыеДанные Цикл
		...
		Если ( CommitSHA = Неопределено ) Тогда
			...
			ТекстСообщения = НСтр( "ru = '%1: отсутствуют настройки маршрутизации.'" );
			ОтправляемыйФайл.ОписаниеОшибки = СтрШаблон( ТекстСообщения, ОписаниеФайла.CommitSHA );
		...
		Иначе
			...
			Если ( ОтправляемыйФайл.АдресаДоставки = Неопределено ) Тогда
				...
				ТекстСообщения = НСтр( "ru = '%1: не задан маршрут доставки файла.'" );
				ОтправляемыйФайл.ОписаниеОшибки = СтрШаблон( ТекстСообщения, ОписаниеФайла.CommitSHA );
...
Процедура ДополнитьЗапросНастройкамиМаршрутизации( ДанныеЗапроса, Знач ДанныеДляОтправки ) Экспорт
	...
	Для каждого Commit Из Commits Цикл
		...
		НастройкиМаршрутизации = КоннекторHTTP.JsonВОбъект( Поток );
		ОбщегоНазначения.ДополнитьКоллекциюТекстомИзПотока( Поток, "json", НастройкиМаршрутизации );
