...
Функция ОтправляемыйФайл()
	...
	Результат.Вставить( "АдресаДоставки" );
...
Функция РаспределитьОтправляемыеДанныеПоМаршрутам( Знач ОтправляемыеДанные, Знач ДанныеЗапроса ) Экспорт
	...
	Для Каждого Данные Из ОтправляемыеДанные Цикл
		
		Если ПустаяСтрока(Данные.Операция) Тогда
		...
		ЗаполнитьЗначенияСвойств(ОтправляемыйФайл, Данные);
		
		Если ПустаяСтрока(ОтправляемыйФайл.ОписаниеОшибки) Тогда
			
			CommitSHA = МаршрутыОтправкиФайлов.Получить(Данные.CommitSHA);
		
			Если CommitSHA = Неопределено Тогда
				
				ОтправляемыйФайл.ОписаниеОшибки = "не найден маршрут доставки для текущего коммита";
...
//			ТекстСообщения = СтрШаблон(НСтр("ru = '%1: отстутствуют настройки маршрутизации.'"), ИдентификаторКоммита);
			...
			Иначе
				...
				Адреса = CommitSHA.Получить(Данные.ПолноеИмяФайла);
				
				Если Адреса = Неопределено Тогда
					
					ОтправляемыйФайл.ОписаниеОшибки = "не найдены адреса доставки";
					
				КонецЕсли;
				
				ОтправляемыйФайл.АдресаДоставки = Адреса;
