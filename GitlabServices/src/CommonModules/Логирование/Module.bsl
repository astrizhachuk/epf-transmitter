...
// 
// Параметры:
// 	Destination - ОбъектМетаданных - объект, в который необходимо загрузить данные журнала регистрации;
...
Процедура ДекораторЗагрузитьИсториюСобытий(Знач Destination,
	...
	Если ОбщегоНазначения.ЭтоСправочник(Destination) Тогда
		
		Если ТабличнаяЧасть = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Если Destination.ТабличныеЧасти.Найти(ТабличнаяЧасть) = Неопределено Тогда
			Возврат;			
		КонецЕсли;
		...
		Если ПараметрыОтбора.Свойство("Данные", Ссылка) Тогда
			Попытка
				Справочники[Destination.Имя].ЗагрузитьИсториюСобытий(Ссылка, ПараметрыОтбора, ДобавленоЗаписей);
			Исключение
				...
				ТекстСообщения = СтрШаблон(ТекстСообщения, Destination.Имя);
