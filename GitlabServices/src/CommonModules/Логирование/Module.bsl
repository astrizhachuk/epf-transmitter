...
Процедура ПодготовитьHTTPСервисОтвет( ДополнительныеДанные, Знач Тип, Знач Сообщение )
	...
	СформироватьТелоЗапроса = КодыСостоянияHTTPКлиентСерверПовтИсп.isOk(HTTPСервисОтвет.КодСостояния)
			ИЛИ КодыСостоянияHTTPКлиентСерверПовтИсп.isForbidden(HTTPСервисОтвет.КодСостояния)
			ИЛИ КодыСостоянияHTTPКлиентСерверПовтИсп.isLocked(HTTPСервисОтвет.КодСостояния);
