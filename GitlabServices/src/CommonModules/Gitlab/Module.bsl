...
Функция ПолучитьФайлыКОтправкеПоДаннымЗапроса( Знач ОбработчикСобытия, ДанныеЗапроса ) Экспорт
	...
	// TODO	
	ИмяФайлаНастроекМаршрутизации = НастройкаСервисов.ТекущиеНастройки().ИмяФайлаНастроекМаршрутизации;
	ДополнитьОписаниеФайловНастройкамиМаршрутизации( Результат, ДанныеЗапроса, ПараметрыПроекта, ИмяФайлаНастроекМаршрутизации );
	// TODO	
...
#КонецОбласти
	...
	// TODO
Процедура ДополнитьОписаниеФайловНастройкамиМаршрутизации( ОписаниеФайлов, Знач ДанныеЗапроса, Знач ПараметрыПроекта, Знач ИмяФайла )
	...
	Для каждого Commit Из Commits Цикл
		...
		Дата = Commit.Получить( "timestamp" );
				...
				ПутьКФайлуRAW = Gitlab.ПутьКФайлуRAW( ПараметрыПроекта.Идентификатор, ИмяФайла, CommitSHA );
				НоваяСтрока.ПутьКФайлуRAW = ПутьКФайлуRAW;
				НоваяСтрока.ПолноеИмяФайла = ИмяФайла;
