...
// Формирование краткого описания сервиса в виде коллекции по метаданным конфигурации (см. API REST сервиса).
// 
// Параметры:
...
Функция ОписаниеСервиса( Знач ИмяСервиса ) Экспорт
	...
	ЗаполнитьОписаниеСервиса( ИмяСервиса, МетаданныеСервиса, Описание );
...
Процедура ЗаполнитьОписаниеСервиса(Знач ИмяСервиса, Знач МетаданныеСервиса, Описание)

	Перем Шаблоны;	
	Перем ОписаниеШаблонов;
	...
	Шаблоны = МетаданныеСервиса.ШаблоныURL;
	ОписаниеШаблонов = ОписаниеШаблоновСервиса( Шаблоны );
	
	Описание.Вставить( "templates", ОписаниеШаблонов );
