...
Заголовки.Вставить("Name", "test");		
    ...
    "http://localhost/target/hs/gitlab/update",
    ДвоичныеДанные, 
...
// * ApiKeyИБПолучателя - Строка - (См. Константа.ApiKeyИБПолучателя);
...
Функция НастройкиСервисовGitLab() Экспорт
	...
	Настройки.Вставить("ApiKeyИБПолучателя", "");
...
//	Настройки.Вставить("МестоположениеСервисаИБПолучателя", "");
	...
	Попытка
		...
		Настройки.ApiKeyИБПолучателя    = ApiKeyИБПолучателя();
...
// * ApiKey - Строка - API-key доступа к сервису.
//
Функция ИнициализироватьПараметрыДоставки() Экспорт
	...
	ПараметрыДоставки.Вставить("ApiKey", 0);
...
// Получает значение константы с API-key, под которым происходит подключение к сервисам
// конечных точек доставки файлов.
// 
// Параметры:
// Возвращаемое значение:
// 	Строка - возвращает API-key пользователя в ИБ-получателях (макс. 20); 
Функция ApiKeyИБПолучателя()
	
	Перем Результат;
	
	Результат = Константы.ApiKeyИБПолучателя.Получить();
...
//		ПараметрыДоставки.Пользователь = ПользовательИБПолучателя();
		ПараметрыДоставки.ApiKey       = ApiKeyИБПолучателя();
