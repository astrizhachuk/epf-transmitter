...
Процедура ЗапуститьОбработкуДанныхВФоне(Знач ИдентификаторWebhook,
	...
	Если ТипЗнч(ДанныеТелаЗапроса) <> Тип("Соответствие") Тогда
		...
		Логирование.Ошибка( "Core.ОбработкаДанныхВФоне", ТекстСообщения, ИдентификаторWebhook );
	...
	Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
		...
		Логирование.Предупреждение( "Core.ОбработкаДанныхВФоне", ТекстСообщения, ИдентификаторWebhook );
	...
	Логирование.Информация("Core.ОбработкаДанныхВФоне.Начало", ТекстСообщения, ИдентификаторWebhook);
...
Заголовки.Вставить("Name", "test");		
	...
	Исключение
		...
		Логирование.Ошибка( "Core.ПередачаФайлаВИБПриемник", ТекстСообщения, ИдентификаторWebhook );
...
Процедура ПолучитьДанныеТелаЗапроса(Знач ИдентификаторWebhook,
	...
	Логирование.Информация("GitLab.ОбработкаЗапроса.Начало", ТекстСообщения, ИдентификаторWebhook);
	...
	Исключение
		...
		Логирование.Ошибка("GitLab.ОбработкаЗапроса", ТекстСообщения, ИдентификаторWebhook );
...
Процедура ПроверитьТелоЗапроса(Знач ИдентификаторWebhook, Знач ДанныеТелаЗапроса, HTTPСервисОтвет) Экспорт
	...
	Логирование.Информация("GitLab.ПроверкаЗапроса.Начало", ТекстСообщения, ИдентификаторWebhook );
	
	Если ДанныеТелаЗапроса = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'Отсутствует тело запроса.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;		
	КонецЕсли;
	
	Если ДанныеТелаЗапроса.Получить("checkout_sha") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент checkout_sha.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	...
	Если project = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент project.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	...
	Если project.Получить("web_url") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент project/web_url.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	...
	Если project.Получить("id") = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент project/id.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	...
	Если commits = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент commits.'");
		Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
		Возврат;
	КонецЕсли;
	...
	Для каждого commit Из commits Цикл
		Если commit.Получить("id") = Неопределено Тогда
			HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
			ТекстСообщения = НСтр("ru = 'В данных запроса отсутствует элемент commits/../id.'");
			Логирование.Ошибка("GitLab.ПроверкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет);
			Возврат;
		КонецЕсли;
	КонецЦикла;
	...
	Логирование.Информация("GitLab.ПроверкаЗапроса.Окончание", ТекстСообщения, ИдентификаторWebhook);
...
Процедура ПроверитьЗапросUpdates(Знач ИдентификаторWebhook,
	...
	Если НЕ ЭтоРепозиторийВнешнихОтчетовИОбработок(HTTPСервисЗапрос) Тогда
		...
		Логирование.Предупреждение( "GitLab.ОбработкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет );
	...
	Если НЕ isUpdates Тогда
		...
		ТекстСообщения = НСтр("ru = 'Не определен метод сервиса, см. описание сервиса ../gitlab'");
		Логирование.Предупреждение( "GitLab.ОбработкаЗапроса", ТекстСообщения, ИдентификаторWebhook, HTTPСервисОтвет );
...
Процедура СохранитьДанныеВИБ(Знач ИдентификаторWebhook, Знач Ключ, Знач Ресурс, Знач Данные)
	...
	Исключение
		...
		Логирование.Предупреждение( "Core.СохранениеДанных", ТекстСообщения, ИдентификаторWebhook );
...
Процедура ФоноваяОбработкаДанныхЗапроса(Знач ИдентификаторWebhook,
	...
	Попытка
		...
		Если ДанныеДляОтправки.Количество() = 0 Тогда
			...
			ТекстСообщения = СтрШаблон(ТекстСообщения, КлючЗапросаGitLab);
			Логирование.Информация( "Core", "ОбработкаДанныхВФоне", ТекстСообщения, ИдентификаторWebhook );
...
//		ПараметрыДоставки.Пользователь = ПользовательИБПолучателя();
		...
		Для каждого ТочкаДоставки Из ДанныеДляОтправки Цикл
			...
			Попытка
				...
				Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
					...
					Логирование.Предупреждение( "Core.ПередачаФайлаВИБПриемник", ТекстСообщения, ИдентификаторWebhook );
		...
		Логирование.Информация("Core.ОбработкаДанныхВФоне.Окончание", ТекстСообщения, ИдентификаторWebhook );
...
Функция ВыполнитьHTTPЗапросНаПолучениеФайла(Знач ИдентификаторWebhook,
	...
	Исключение
		...
		Логирование.Ошибка( "Core.ПолучениеДанныхДляОтправкиФайлов", ТекстСообщения, ИдентификаторWebhook );
	    //ВызватьИсключение;
	КонецПопытки;
...
Функция ПодготовитьДанныеКОтправке(Знач ИдентификаторWebhook, Знач Параметры)
	...
	Логирование.Информация("Core.ПодготовкаДанных.Начало", ТекстСообщения, ИдентификаторWebhook);
	...
	ТекстСообщения = НСтр("ru = '%1: подготовка данных к отправке.'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Параметры.КлючЗапросаGitLab);
	Логирование.Информация("Core.ПодготовкаДанных.Окончание", ТекстСообщения, ИдентификаторWebhook );
...
Функция ПолучитьДанныеДляОтправкиССервераGitLab(Знач ИдентификаторWebhook,
	...
	Для каждого ОписаниеФайла Из ДанныеДляОтправки Цикл
		...
		Если HTTPОтвет.КодСостояния = 200 Тогда
			...
			Логирование.Информация("Core.ПолучениеДанныхДляОтправкиФайлов", ТекстСообщения,	ИдентификаторWebhook, HTTPОтвет);
		...
		Иначе
			...
			Логирование.Ошибка("Core.ПолучениеДанныхДляОтправкиФайлов", ТекстСообщения, ИдентификаторWebhook, HTTPОтвет);
...
Процедура СкорректироватьТочкиДоставкиФайлов(Знач ИдентификаторWebhook, Знач Коммиты, ТочкиДоставки)
	...
	Для каждого Коммит Из Коммиты Цикл
		...
		Если Настройки = Неопределено Тогда
			...
			Логирование.Информация("Core.ФормированиеМаршрутовДоставки", ТекстСообщения, ИдентификаторWebhook);
			Продолжить;
