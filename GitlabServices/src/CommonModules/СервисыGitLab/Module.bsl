...
Функция СформироватьЗапросНаMergeRequests(Знач ДанныеТелаЗапроса) Экспорт
	...
	Запрос.Вставить("СекретныйКлюч", НастройкиСервисов.GitLabUserPrivateToken());
...
Процедура ДополнитьДанныеЗапросаНастройками(Знач ИдентификаторWebhook,
	...
	PrivateToken           = НастройкиСервисов.GitLabUserPrivateToken();
...
Функция ОписаниеФайловНастроекМаршрутизации(Знач ДанныеТелаЗапроса, Знач ПараметрыПроекта)
	...
	ИмяФайлаНастроек     = НастройкиСервисов.ИмяФайлаНастроекМаршрутизации();
...
//		ПараметрыДоставки.Таймаут = ТаймаутДоставкиПолучатель();
		
		ПараметрыДоставки = ПередачаДанных.ПараметрыДоставки(); 
...
Функция ПодготовитьДанныеКОтправке(Знач ИдентификаторWebhook, Знач Параметры)
	...
	Таймаут = НастройкиСервисов.ТаймаутGitLab();
