...
Функция ОписаниеФайловНастроекМаршрутизации(Знач ДанныеТелаЗапроса, Знач ПараметрыПроекта)
	...
	Для каждого Коммит Из ДанныеТелаЗапроса.Получить("commits") Цикл
		...
		ПутьКФайлу = ОтносительныйПутьКФайлуRAW(ИдентификаторПроекта, ИдентификаторКоммита, ИмяФайлаНастроек);
...
Функция ОтносительныйПутьКФайлуRAW(Знач ИдентификаторПроекта, Знач Коммит, Знач ПолноеИмяФайла)
...
// TODO в переработку, проблема с URL
...
Функция ПолучитьДанныеДляОтправкиССервераGitLab(Знач ИдентификаторWebhook,
	...
	Для каждого ОписаниеФайла Из ДанныеДляОтправки Цикл
		...
		// TODO собрать урл из структуры, либо пробросить
		СтруктураФайл = Gitlab.ПолучитьФайл("http://gitlab", ОписаниеФайла.ПутьКФайлу, PrivateTokenGitLab, Таймаут);
...
Процедура ДополнитьТаблицуОтправкиДанныхИзменениямиРепозитория(Знач ИменаФайлов,
	...
	Для каждого ПолноеИмяФайла Из ИменаФайлов Цикл
		...
		ПутьКФайлу  = ОтносительныйПутьКФайлуRAW(Параметры.Проект.Идентификатор,
