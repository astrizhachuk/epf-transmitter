...
//		КонецЦикла;
		...
		ПараметрыСоединения = Gitlab.ПараметрыСоединения("http://gitlab"); // заглушка
		Gitlab.ЗаполнитьОтправляемыеДанныеФайлами(ДанныеДляОтправки, ИдентификаторWebhook, ПараметрыСоединения);	
...
//	
//	ПараметрыСоединения = Gitlab.ПараметрыСоединения("http://gitlab"); // заглушка
//	ПутиКФайлам = ДанныеДляОтправки.ВыгрузитьКолонку("ПутьКФайлу");
//	ПолученныеФайлы = Gitlab.ПолучитьФайлы(ПараметрыСоединения, ПутиКФайлам);
//	
//	Для каждого ОписаниеФайла Из ПолученныеФайлы Цикл
//			
//		Если ОписаниеФайла.ОписаниеОшибки = Неопределено Тогда
//			
//			НайденнаяСтрока = ДанныеДляОтправки.Найти(ОписаниеФайла.ПутьКФайлуRAW, "ПутьКФайлу");
//			Если НайденнаяСтрока <> Неопределено Тогда
//				
//				НайденнаяСтрока.ИмяФайла = ОписаниеФайла.ИмяФайлаUTF8;
//				НайденнаяСтрока.ДвоичныеДанные = ОписаниеФайла.ДвоичныеДанные;
//				
//				ТекстСообщения = СтрШаблон(НСтр("ru = '%1: с сервера GitLab получен файл %2.'"),
//					Коммит,	НайденнаяСтрока.ПолноеИмяФайла);
//					
...
//		Иначе
//			
//			Логирование.Ошибка( "Core.ПолучениеДанныхДляОтправкиФайлов", ОписаниеФайла.Ошибка, ДополнительныеПараметры );	
//			
//		КонецЕсли;
//
//	КонецЦикла;
...
//		
//
//		СтруктураФайл = Gitlab.ПолучитьФайл( ПараметрыСоединения, ОписаниеФайла.ПутьКФайлу );
//		//СтруктураФайл = Gitlab.ПолучитьФайл(ПараметрыПроекта.СтруктураURI.URLСервера, ОписаниеФайла.ПутьКФайлу, PrivateTokenGitLab, Таймаут);
//		
//		Если СтруктураФайл.Ошибка = Неопределено Тогда
//			
//			ОписаниеФайла.ИмяФайла = СтруктураФайл.ИмяФайла;
//			ОписаниеФайла.ДвоичныеДанные = СтруктураФайл.Данные;
//			
//			ТекстСообщения = СтрШаблон(НСтр("ru = '%1: с сервера GitLab получен файл %2.'"),
...
//		Иначе
//			
//			Логирование.Ошибка( "Core.ПолучениеДанныхДляОтправкиФайлов", СтруктураФайл.Ошибка, ДополнительныеПараметры );	
//			
//		КонецЕсли;
//		
//	КонецЦикла;
...
Процедура ДополнитьТаблицуОтправкиДанныхИзменениямиРепозитория(Знач ИменаФайлов,
	...
	Для каждого ПолноеИмяФайла Из ИменаФайлов Цикл
		...
		ПутьКФайлу  = Gitlab.ПутьКФайлуRAW(Параметры.Проект.Идентификатор,
															ПолноеИмяФайла,
