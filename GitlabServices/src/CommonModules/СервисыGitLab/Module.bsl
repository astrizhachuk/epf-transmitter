...
Процедура ЗапуститьОбработкуДанныхВФоне(Знач ИдентификаторWebhook,
	...
	Если ТипЗнч(ДанныеТелаЗапроса) <> Тип("Соответствие") Тогда
		...
		Логирование.Ошибка("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
	...
	Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
		...
		Логирование.Предупреждение("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
	...
	Логирование.Информация("Core", "ОбработкаДанныхВФоне.Начало", ИдентификаторWebhook, ТекстСообщения);
...
Заголовки.Вставить("Name", "test");		
	...
	Исключение
		...
		Логирование.Ошибка("Core", "ПередачаФайлаВИБПриемник", ИдентификаторWebhook, ТекстСообщения);
...
Процедура ПроверитьТелоЗапроса(Знач ИдентификаторWebhook, Знач ДанныеТелаЗапроса, HTTPСервисОтвет) Экспорт
	...
	Если commits = Неопределено Тогда
		HTTPСервисОтвет = Новый HTTPСервисОтвет(400);
		Логирование.Ошибка("ПроверкаЗапроса",
										 "ОбработкаЗапроса",
										 ИдентификаторWebhook,
...
Процедура СохранитьДанныеВИБ(Знач ИдентификаторWebhook, Знач Ключ, Знач Ресурс, Знач Данные)
	...
	Исключение
		...
		Логирование.Предупреждение("Core", "СохранениеДанных",
...
Процедура ФоноваяОбработкаДанныхЗапроса(Знач ИдентификаторWebhook,
	...
	Попытка
		...
		Если ДанныеДляОтправки.Количество() = 0 Тогда
			...
			ТекстСообщения = СтрШаблон(ТекстСообщения, КлючЗапросаGitLab);
			Логирование.Информация("Core", "ОбработкаДанныхВФоне", ИдентификаторWebhook, ТекстСообщения);
...
//		ПараметрыДоставки.Пользователь = ПользовательИБПолучателя();
		...
		Для каждого ТочкаДоставки Из ДанныеДляОтправки Цикл
			...
			Попытка
				...
				Если ЗначениеЗаполнено(АктивныеЗадания) Тогда
					...
					Логирование.Предупреждение("Core", "ПередачаФайлаВИБПриемник", ИдентификаторWebhook, ТекстСообщения);
		...
		Логирование.Информация("Core", "ОбработкаДанныхВФоне.Окончание", ИдентификаторWebhook, ТекстСообщения);
...
Функция ВыполнитьHTTPЗапросНаПолучениеФайла(Знач ИдентификаторWebhook,
	...
	Исключение
		...
		Логирование.Ошибка("Core", "ПолучениеДанныхДляОтправкиФайлов",
								 ИдентификаторWebhook,
...
Функция ПодготовитьДанныеКОтправке(Знач ИдентификаторWebhook, Знач Параметры)
	...
	Логирование.Информация("Core", "ПодготовкаДанных.Начало", ИдентификаторWebhook, ТекстСообщения);
	...
	ТекстСообщения = НСтр("ru = '%1: подготовка данных к отправке.'");
	ТекстСообщения = СтрШаблон(ТекстСообщения, Параметры.КлючЗапросаGitLab);
	Логирование.Информация("Core", "ПодготовкаДанных.Окончание", ИдентификаторWebhook, ТекстСообщения);
