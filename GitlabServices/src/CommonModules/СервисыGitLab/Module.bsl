...
// 
// Параметры:
// 	HTTPЗапрос - HTTPЗапрос, HTTPСервисЗапрос - ссылка на HTTP-запрос.
// Возвращаемое значение:
// 	СправочникСсылка.ОбработчикиСобытий - ссылка на элемент справочника с обработчиками событий;
//		если обработчик не найден, то возвращается пустая ссылка.
//
// @UnitTest
//
Функция НайтиОбработчикСобытияПоHTTPЗапросу( Знач HTTPЗапрос ) Экспорт
	...
	Если ( ТипЗнч(HTTPЗапрос) <> Тип("HTTPСервисЗапрос") И ТипЗнч(HTTPЗапрос) <> Тип("HTTPЗапрос") ) Тогда
		
		Возврат Результат;
								
	КонецЕсли;
	
	СекретныйТокен = HTTPЗапрос.Заголовки.Получить( "X-Gitlab-Token" );
	...
	Возврат Справочники.ОбработчикиСобытий.НайтиПоСекретномуТокену( СекретныйТокен ); 
