...
Функция WebhooksPOST( Запрос )
	...
	Если ( КодыСостоянияHTTPКлиентСерверПовтИсп.isOk(Ответ.КодСостояния) ) Тогда
		...
		ПараметрыЛогирования = Логирование.ДополнительныеПараметры( ОбработчикСобытия, Ответ );
...
// 	ДанныеЗапроса - Соответствие - (исходящий параметр) преобразованное в коллекцию тело запроса; исходный текст тела запроса
