...
Функция WebhooksPOST( Запрос )
	...
	ДесериализоватьТелоЗапроса( ОбработчикСобытия, Запрос, Ответ, ДанныеТелаЗапроса );
...
// 	Данные - Соответствие - (исходящий параметр) преобразованное в коллекцию тело запроса; исходный текст тела запроса
//		добавляется в структуру с ключом "json".
//
Процедура ДесериализоватьТелоЗапроса( Знач ОбработчикСобытия, Знач Запрос, Знач Ответ, Данные = Неопределено )
